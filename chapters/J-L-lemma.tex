...
\section{高维几何} \label{sec:high-dimensional-geometry}
...
\begin{theorem}\label{theorem:volume-surface-area}
...
接下来，我们再说明，如果把$n$维球和$n$维超立方体放在一起看，他们的质量分布也是非常反直觉的. 对于$n \in \N$，$\epsilon>0$，我们定义一个$n$维的“球壳”$A_{n,\epsilon}$：

\[A_{n,\epsilon} = \{x \in \R^n:(1-\epsilon)\sqrt{n/3}<\norm{x}<(1+\epsilon)\sqrt{n/3}\}.\]
这是一个半径为$\sqrt{n/3}$的$n$维“球壳”，相对厚度大约为$2\epsilon$. 
...
\begin{itemize}
    ...
    \item 当$n$很大时，超立方体$(-1,1)^n$的绝大部分体积都是由一个相对厚度约为$\epsilon$，半径为$\sqrt{n/3}$的$n$维球壳提供的，当$\epsilon$很小时，这个球壳非常薄. 一层\textit{薄}球壳占据了一个\textit{实心}立方体的绝大部分体积，这个在二维和三维空间中也是难以想象的. 
\end{itemize}
...
由弱大数定律（\Cref{theorem:khinchin}），$\sum_{i=1}^n X_i^2/n$偏离期望$1/3$某个值的概率会随着$n$趋于无穷而趋于零. 更精确来说，对任意$\epsilon'>0$，当$n\to \infty$时，
\[
\Pr\left[\left|\frac{\sum_{i=1}^n X_i^2}{n}-\frac{1}{3}\right|>\epsilon'\right]\to 0.
\]
变形得
\[
\Pr\left[\sqrt{\left(\frac{1}{3}-\epsilon'\right)n}\leq \sqrt{\sum_{i=1}^nX_i^2}\leq\sqrt{\left(\frac{1}{3}+\epsilon'\right)n}\right]\to 1.
\]
对任意$\epsilon>0$，可以取恰当的$\epsilon'$使得
\[
(1-\epsilon)\sqrt{\frac{n}{3}}\leq \sqrt{\left(\frac{1}{3}-\epsilon'\right)n},\quad (1+\epsilon)\sqrt{\frac{n}{3}}\geq \sqrt{\left(\frac{1}{3}+\epsilon'\right)n}.
\]
于是，
\[
...
\begin{theorem}[Stein悖论]\label{theorem:stein-paradox}
...
现在，我们回到最早的估计问题，我们把$\mu$看作是圆心$c$，而随机采样的点就看作是样本点. 上面的概率不等式意味着，随着维数变高，朴素估计量$x$与真正的$\mu$之间的差距会越来越大. 不仅如此，朴素估计量对$\mu$的估计会偏大. 

直觉上，要想更加精确估计$\mu$，我们需要比样本点$x$更接近原点. 仔细观察 \eqref{eq:James-Stein-estimation}，James-Stein估计量的确是这样做的. 下面，我们详细介绍这一估计量的几何推导. 

由于坐标系的选取是随意的，可以设一条坐标轴和$\mu$的方向相同，其他$n-1$根坐标轴方向随意，但正交. 在新坐标系下，
\[\mu = (\norm{\mu},0,\dots,0)^\t.\] 

设坐标变换后的样本是$x=(x_1,x_2,\dots,x_n)^\t$. 损失函数可以被分解成两个部分的和：
\begin{equation}
...
在机器学习中，大多数问题都可以转化为参数估计问题——也就是寻找最优的模型参数（比如神经网络的权重）. 虽然表面上我们可以任意选择参数空间的“原点”（即参数的参考点），但这些选择并非完全等价，而是隐含了人为的先验假设. 

例如，当我们将神经网络的所有权重初始化为零时，无论输入什么数据，网络都只会输出零；但如果把所有权重初始化为1作为“原点”，这种输出全零的特性就不复存在了. 类似的现象也出现在统计学中的Stein悖论中，该理论表明，适当地将参数估计值向全零的方向收缩，往往能提升估计的准确性. 

在机器学习中，我们同样有偏好“原点”的倾向，例如使用$L^2$正则化这样的技术可以通过约束模型参数的范围来抑制其学习能力，从而降低过拟合的风险. 本节的内容通过极端的例子展示了正则化背后的原理：在高维空间中，离原点较远的地方体积远大于靠近原点的地方. 因此，在高维空间中，向原点收缩参数值就能显著减少模型参数的搜索空间. 
...
\begin{quotation}
    \textit{“远有潜龙，勿用. ”}\footnote{原句出自《周易·乾卦》，“初九：潜龙勿用. ”孔子对这句话的解释是如果身居下位，时机还没有成熟，应当像潜藏的龙一样不要施展你的才干. 这里，复杂的、远离原点的模型就像是潜龙，隐藏着巨大的力量，但是现在人类对他们的理解还远远不够，因此我们应当保持谨慎，不要轻易使用. }
\end{quotation}
...
\begin{definition}[示性函数]\label{definition:indicator-function}
...
从定义就可以得到如下基本性质：
\begin{proposition}\label{proposition:indicator-function}
    设$A, B$是两个事件，则
    \begin{enumerate}
        \item $I(AB) = I(A) I(B)$.
        \item $I(A)^2 = I(A)$.
...
\begin{proposition}\label{proposition:expectation-of-indicator-function}
    设$A$是一个事件，则
    \[
        \E[I(A)] = \Pr(A).
    \]
...
示性函数可以把对概率的计算变成对期望的计算. 回忆期望的线性性（见\Cref{proposition:expectation-property}）：设$a,b\in\R$，$X,Y$是有期望的随机变量，那么成立
\[\E[aX+bY]=a\E[X]+b\E[Y].\]
...
\begin{theorem}[Markov不等式]\label{theorem:markov-inequality}
...
\begin{corollary}[Chebyshev不等式]\label{corollary:chebyshev-inequality}
...
\begin{theorem}[Hoeffding 不等式]\label{theorem:hoeffding-inequality}
...
\begin{theorem}[Hoeffding 不等式，一般情形]\label{theorem:hoeffding-inequality-general}
...
下面我们介绍 Chernoff 不等式. 
\begin{theorem}[Chernoff 不等式]\label{theorem:chernoff-inequality}
...
\begin{theorem}[Johnson-Lindenstrauss 引理]\label{theorem:johnson-lindenstrauss-lemma}
...
\begin{proposition}\label{proposition:gaussian-vector}
假设$u\sim\Nor(\mu,\Sigma)$是一个$n$维正态向量，$M$是一个$m\times n$矩阵，那么$Mu$是一个$m$维正态向量，并且$Mu\sim \Nor(M\mu,M\Sigma M^T)$.
\end{proposition}
...
\end{lemma}
\begin{proof}
    将$A$视作一个$mn$维的正态向量，注意到，$(Au)_i = \sum_{j=1}^m A_{ij}u_j$，所以$Au$是一个从向量$A$线性变换得到的向量. 根据\Cref{proposition:gaussian-vector}，$Au$是一个正态向量，只需计算它的期望和协方差矩阵. 
...
J-L引理实际上在说，$\norm{Au}_2$与$1$的偏差超过某个$\epsilon$的概率是非常小的. 于是，为了证明J-L引理，我们最重要的任务是给出$Au$这样向量模长的集中不等式：
...
有了单位模引理，我们就可以很容易证明J-L引理了. 将\Cref{lemma:gaussian-vector-diff} 中的$Au$带入单位模引理，得到
\[
...
\begin{theorem}[J-L引理，内积形式]\label{theorem:johnson-lindenstrauss-lemma-inner-product}
...
\begin{example}[词向量维度]\label{example:word-vector-dimension}
...
为了表示这种相似性，在压缩过的词向量空间中，所有词之间的内积应该尽量保持词本身的相似度. 这正是内积形式的J-L引理（\Cref{theorem:johnson-lindenstrauss-lemma-inner-product}）所描述的情况. 于是，J-L引理告诉我们，$\O(\log N)$维空间足以容纳下$N$个单词，还保持了单词之间的相似性. 

到此时，我们要十分警惕“理论指导实践”这一表述的理解. J-L引理（理论）所指导的结论，成立的前提是“正态随机矩阵”，然而我们完全不清楚我们的压缩是否符合这一条件. 所以，我们不能说J-L引理直接给了我们合适的词向量维度，我们只能说J-L引理给了词向量选择的一个直觉. 具体应该用多少维度的词向量，还需要实验来验证.
\end{example}
...
现在，假设数据库中的所有键、值对是$\{k_i:v_i\}_{i=1}^N$，我们希望从中找到与某个查询$q$最接近的键对应的值. 在深度学习中，键、值、查询都可以按照\Cref{example:word-vector-dimension} 的方式表示成向量. 类似地，我们可以用内积来衡量键和查询的相似度. 

假设这些查询和键的向量都处在$\R^d$中，那么注意力的计算公式为
\[
    a_j = \frac{\e^{\inner{q}{k_j}}}{\sum_{j=1}^N \e^{\inner{q}{k_j}}}.
\]
换言之，我们把相似性转化为了概率分布，相似度越高的键，被选中的概率越大. 我们把$d$称为\textit{注意力头大小}，在很多深度学习框架中，它被记为 \verb#head_size#.
...
类似\Cref{example:word-vector-dimension} 的问题，如果真实世界中$a_{ij}$是$p_{ij}$，我们应该如何选择向量维数$d$才能保证$a_{ij}$能够足够好地逼近$p_{ij}$呢？这个问题和\Cref{example:word-vector-dimension} 是一样的.

在这个例子中，词向量的维度变成了$d$，词表大小变成了数据库中的键值对数量$N$. J-L引理告诉我们的答案依然是只需要$\O(\log N)$的空间就足以容纳下$N$个键值对，还能保持多组查询与键之间的相似性. 

更为重要的是，这个压缩空间的维度$d$只和词表大小有关，而和查询的数量无关，这可以说明两点：
\begin{itemize}
    \item 如果有同样多的参数，头很大的单头注意力机制并不如头比较小的多头注意力机制. 大的单头注意力，维数太高，但J-L引理告诉我们，不需要那么多维数的东西. 但是另一方面头较小的多头注意力，可以用更多查询来查询数据库，因而可以更好地利用数据库中的信息.
    \item 无论多少个头，多头注意力的$d$并不需要随着头的数量增加而显著增加. 
\end{itemize}

以上两点和现实中大模型的设计是非常符合的. 一般来说，大语言模型的多头注意力机制中，注意力头大小维持在$128$到$256$之间，而头的数量会随着模型的增大而显著增加. 这不仅仅是从计算效率上考虑的，实际上，这种设计也符合J-L引理的结论.
\end{example}
...
\begin{enumerate}[wide,labelindent=0pt]
    \item \label{exercise:volume-surface-area} 证明\Cref{theorem:volume-surface-area}. 
    \item *\label{exercise:stein-paradox}（\cite{jamesEstimationQuadraticLoss1961}）本题证明\Cref{theorem:stein-paradox}. 首先，定义$X=(x_1,\dots,x_n)^\t$，然后定义
    \[\phi(X)=\left(1-\frac{b}{\norm{X}^2}\right)X.\]
    \begin{enumerate}
        \item 证明，均方误差具有如下分解：
        ...
        因此，当$b=n-2$时，我们就证明了\Cref{theorem:stein-paradox}.
    \end{enumerate}
    ...
    \item \label{exercise:hoeffding-inequality-general} 证明\Cref{theorem:hoeffding-inequality-general}. 
