...
\textit{模态词}是指说话者对语句的限定. 不同的限定反映了不同的逻辑状态. 例如：\textit{必然}、\textit{可能}、\textit{过去}、\textit{未来}、\textit{知识}、\textit{信念}和\textit{可证明}等，都是典型的模态概念. 比如，我们可以说“明天可能会下雨”、“我知道明天会下雨”、“我相信明天会下雨”等等，他们要表达的意思并不是经典逻辑可以表述的. 
...
不同于经典的命题逻辑，我们经常需要定义不同的模态语言，为了方便，我们引入一种更简洁的记号：
...
\[\Box_a\phi\wedge\Box_a\psi\to\Box_a(\phi\wedge\psi).\]
...
形式化地说，$\Box\phi$在世界$w$上成立当且仅当$\phi$在$w$的所有后继（也就是从$w$出发的边指向的节点）上为真. 这种语义通常被称为\textit{Kripke语义}或\textit{可能世界语义}. 在一个世界讨论可能与必然的时候，会取决于它与其他世界的联系. 

我们再来看一个例子. 

\begin{example}\label{example:kripke-model-basic}
...
有了对概念的自然语言描述和例子，接下来，我们形式上给出模态逻辑的模型和语义定义. 
...
比如，在\Cref{example:kripke-model-basic} 中，我们讨论了$\phi:\Box (p\to\Box q)$在一个Kripke模型中每个可能世界上的真值. 在例子的计算中，我们发现，所有可能世界上$\phi$都是真的. 所以，我们其实可以说，$\phi$在整个Kripke模型上也是真的. 
...
更抽象地来看，\Cref{example:kripke-model-basic} 其实给出了左上角的一个例子，下面，我们可以演示右下角的一个例子. 

\begin{example}\label{example:modal-logic-frame-validity}
...
\begin{example}\label{example:box-top}
...
此外，结合这个例子与\Cref{example:box-top}，我们会发现，一个框架的特定性质可以由多个不同的公式定义. 
\end{example}
...
\begin{proposition}[知识公理，真理公理]\label{proposition:knowledge-axiom}
...
\begin{proposition}[内省公理]\label{proposition:knowledge-introspection}
正内省公理：
...
\begin{proposition}[一致性公理]\label{proposition:knowledge-consistency}
$\vDash_H\neg K_i\bot$.
\end{proposition}
...
\begin{proposition}[不动点公理]\label{proposition:common-knowledge-fixed-point}
\[\vDash C\phi\leftrightarrow E(\phi\wedge C\phi).\]
\end{proposition}

\begin{proposition}[归纳规则]\label{proposition:common-knowledge-induction}
如果
...
\begin{proposition}\label{proposition:consistent-decision}
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    \item \label{exercise:epistemic-logic-axioms} 证明\Cref{proposition:knowledge-axiom}、\Cref{proposition:knowledge-introspection} 和\Cref{proposition:knowledge-consistency}.
    ...
    \item \label{exercise:common-knowledge-axioms} 证明\Cref{proposition:common-knowledge-fixed-point} 和\Cref{proposition:common-knowledge-induction}.
    ...
    \item \label{exercise:consistent-decision} 证明\Cref{proposition:consistent-decision}.
