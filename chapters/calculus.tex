...
\begin{example}\label{example:metric}
...
\begin{theorem}\label{theorem:metric-norm}
...
\begin{proposition}\label{proposition:lp-norm}
...
\begin{proposition}\label{proposition:lp-metric}
...
\begin{proposition}\label{proposition:open-ball}
...
我们给几个开集的例子：
\begin{example}[范等价拓扑空间]\label{example:lp-topology}
...
\begin{example}[乘积拓扑空间]\label{example:product-topology}
...
\begin{proposition}\label{proposition:open-prop}
    设$(X,d)$是一个非空度量空间，则
    \begin{enumerate}
...
注意，开集只对有限交封闭. 可以看一个简单的例子：
\[\bigcap_{n=1}^\infty(-1/n,1/n)=\{0\},\]
但是$\{0\}$不是开集，因为这个集合不可能包含任何开球. 

\Cref{proposition:open-prop} 其实就是一般拓扑空间中开集要满足的三条公理. 我们之所以将它写为命题，是因为我们的开集定义基于度量空间，而非一般的拓扑空间.
...
\begin{proposition}\label{proposition:closed-prop}
    设$(X,d)$是一个非空度量空间，则
    \begin{enumerate}
...
\begin{proposition}[Heine-Borel有限覆盖原理]\label{proposition:heine-borel}
...
\begin{proposition}\label{proposition:converge-ball}
...
\begin{itemize}
    ...
    \item 在$\R^n$和$L^p$范数下，$\{x_k\}_{k=1}^\infty$收敛到$x$，当且仅当对任意$i=1,\dots,n$，$\{x_k^i\}_{k=1}^\infty$收敛到$x^i$，其中，
    \[x_k=(x_k^1,\dots,x_k^n),\quad x=(x^1,\dots,x^n).\]
    这个例子表明，高维空间中的收敛性可以从每个分量看. 
...
\begin{theorem}\label{theorem:compact-converge}
    设$(X,d)$是一个度量空间，$F\subseteq X$，则$F$是紧集当且仅当$F$中的任意序列都有收敛子列. 
\end{theorem}

这一定理的证明并不算困难，但是需要陈述的事实较多，且与本书关联不大，所以这里都略去. 

\Cref{theorem:compact-converge} 足以表明紧集这一概念的重要性了. 然而，这一定理的成立只需要度量空间，度量空间是一个非常弱的概念，我们关心的$\R^n$空间实际上有更强的性质，这一性质是\textit{完备性}. 要定义完备性，我们需要\textit{Cauchy列}. 
...
\begin{proposition}\label{proposition:cauchy-converge}
...
\begin{theorem}\label{theorem:compact-bounded}
...
\begin{theorem}\label{theorem:continuous-open}
...
这一命题的证明非常类似\Cref{proposition:open-ball}，我们这里就不给出了. 注意，极限的开集定义所用的集合$V\setminus\{x_0\}$也是一个开集，它是$x_0$的去心邻域，所以这一定义确实是纯拓扑的. 

连续映射的定义也可以完全由拓扑给出：

\begin{theorem}\label{theorem:continuous-topology}
...
利用\Cref{theorem:continuous-open}、\Cref{proposition:closed-prop} 以及开集的定义，很容易证明这一命题，这里不再赘述. 

\begin{example}\label{example:metric-norm-continuous}
在不给任何额外定义的时候，我们有一个非常自然的连续映射的例子，那就是\textbf{度量}. 设$(X,d)$是一个度量空间，我们证明度量$d:X\times X\to\R$是一个连续函数. 

我们利用点连续的定义，证明$d$在每一点都连续. 设$(x_1,y_1)\in X\times X$. 我们利用\Cref{theorem:continuous-open} 和原始定义的混合版本. 注意到要证明所有包含$d_0=d(x_1,y_1)$的开集$U$满足条件，根据$U$的构造，只需要证明，对任意$\epsilon>0$，$B(d_0,\epsilon)$满足条件. 为此，取一个包含$(x_1,y_1)$的开集$V=B(x_1,\epsilon/2)\times B(y_1,\epsilon/2)$（关于这个为什么是开集，详细讨论见\Cref{example:product-topology}），则对任意$(x_2,y_2)\in V$，有$d(x_1,x_2)<\epsilon/2$，$d(y_1,y_2)<\epsilon/2$，所以根据三角不等式，
...
\begin{theorem}\label{theorem:limit-converge}
...
\begin{corollary}\label{corollary:continuous-converge}
    设$(X,d_X)$和$(Y,d_Y)$是两个度量空间，$f:X\to Y$是一个映射，则下列表述等价：
    \begin{enumerate}
        \item $f$在$x\in X$连续. 
...
其次，连续对复合是封闭的：
\begin{proposition}\label{proposition:continuous-composition}
...
\begin{corollary}\label{corollary:continuous-linear}
    设$(X,\norm{\cdot}_X)$是赋范空间，则数乘是$X\to X$的连续映射，向量加法是$X\times X\to X$的连续映射. 因此，有限维线性空间到有限维线性空间的线性映射都是连续映射. 
\end{corollary}

根据\Cref{corollary:continuous-converge}，这一结论也有对应的序列版本，我们就不再列出了. 特别要注意的是，这一结论也适用于$\R$. 将乘法$\times:\R\times\R\to\R$和除法$\div:\R\times(\R\setminus\{0\})\to\R$看成向量空间$\R$上的数乘运算，于是他们也都是连续映射\footnote{尽管从证明的逻辑顺序来说，应该是先有了实数的四则运算连续性，然后才有了赋范空间的连续性. 我们这样写是为了避免将类似的结论重复讲多次.  }. 

最后，连续意味着有界：
\begin{proposition}\label{proposition:continuous-bounded}
...
\begin{proposition}\label{proposition:continuous-compact}
...
\begin{proposition}[单调有界原理]\label{proposition:monotone-bounded}
    设$\{x_n\}$是一个单调有界的实数列，则$\{x_n\}$收敛. 
\end{proposition}

接下来一个是说有上（下）界的实数集一定有上（下）确界，即最小可能的上（下）界是一个确实存在的实数，这也是一种完备性的体现. 
\begin{proposition}[确界原理]\label{proposition:supremum}
    设$A\subseteq\R$，如果$A$有上界，则$\sup A$存在；如果$A$有下界，则$\inf A$存在. 
\end{proposition}

确界原理给了一种求确界的方式：
\begin{proposition}\label{proposition:supremum-epsilon}
...
\begin{itemize}
    ...
    \item $n\to\infty$时，$\sum_{k=1}^n\frac{1}{n}\sim\log n$.
\end{itemize}
\end{example}

最后，我们回到连续的整体性质上来. 首先是Weierstrass最值定理：

\begin{theorem}[Weierstrass最值定理]\label{theorem:weierstrass}
    紧集上的连续函数$f:F\to\R$在该紧集$F$的某个点取最大（最小）值. 
\end{theorem}

然后是介值定理：
\begin{theorem}[介值定理]\label{theorem:intermediate}
...
\begin{proposition}\label{proposition:derivative}
...
\begin{proposition}\label{proposition:uni-derivative-property}
    ...
    \begin{itemize}
        ...
        \item 如果$f$存在反函数$f^{-1}$，则$f^{-1}$在$f(x_0)$处可微，且
...
\begin{proposition}\label{proposition:derivative-geometric}
...
\Cref{proposition:derivative-geometric} 提供了这样一种视角：微分号$\d$相当于把$\R\to\R$的函数变成了另外一个$\R\to\R$的函数（即切映射），同时保持函数复合运算的单位元（$\id$）、复合和逆元关系. 利用这一性质，我们可以用更加代数的方法研究微分（切函子），但这超出了本书的范围，我们就不再详细讨论了.
...
\[
    \frac{\d^n y}{\d x^n}=\underbrace{\frac{\d}{\d x}\dots\frac{\d}{\d x}}_{n\text{个}}y.
\]
...
\begin{definition}[极大值，严格极大值，极小值，严格极小值]\label{definition:extremum}
...
\begin{theorem}[Lagrange中值定理]\label{theorem:lagrange-mid}
...
\begin{theorem}[Lagrange有限增量定理]\label{theorem:lagrange-finite}
...
\]
容易求出，$a_0=f(x_0)$，$a_1=f'(x_0)$，$a_2=\frac{f''(x_0)}{2}$，$a_3=\frac{f'''(x_0)}{6}$，$\dots$，$a_k=\frac{f^{(k)}(x_0)}{k!}$，因此我们得到了Taylor公式：
...
\begin{theorem}\label{theorem:extreme}
    设$f:(a,b)\to\R$在$x_0$处$n$次可微，且$f'(x_0)=f''(x_0)=\dots=f^{(n-1)}(x_0)=0$，$f^{(n)}(x_0)\neq 0$，则
    \begin{itemize}
...
接下来的问题就是，如何表示线性映射$\d f_x$？我们先从实值函数开始. 考虑$\R^n$的标准正交基$\{e_i\}_{i=1}^n$，它也是切空间$T\R^n_x$的标准正交基，根据Riesz表示定理（\Cref{theorem:riesz}），存在一个向量$g$，使得
\[
    \d f_x h=\inner{g}{h}.
\]
...
\[f(x+te_i)=f(x)+\d f_x(te_i)+o(te_i)=f(x)+g_it+o(t).\]
因此，
...
\begin{proposition}\label{proposition:directional-derivative}
...
接下来我们讨论向量值函数微分的表示问题. 选取$\R^n$（也就是$T\R_x^n$）的标准正交基$e_i$，选取$\R^m$（也就是$T\R^m_{f(x)}$）的标准正交基$e_i$，则根据\Cref{sec:matrix}的讨论，我们可以用一个$m\times n$的矩阵来表示$\d f_x$，这个矩阵被称为$f$在$x$处的\textbf{Jacobi矩阵}，记作$J_f(x)$. 

下面我们计算$J_f(x)$的具体表示. 假设$f(x)$的坐标是
\[(f_1(x),\dots,f_m(x))^\t,\]
考虑$h\in T\R_x^n$，它的坐标是$(h_1,\dots,h_n)^\t$，$\d f_x h$的坐标应该是
\begin{align*}
    \begin{pmatrix}
        \d f_{1,x}h \\
        \vdots \\
...
在\Cref{example:polar-coordinates} 中我们会看到，Jacobi行列式表明了坐标变换时相应体积变化的比率. 这一事实使得它在积分学的变量替换中有着核心作用. 
...
\begin{proposition}\label{proposition:derivative-property}
...
\begin{example}\label{example:quadratic-derivative}
考虑二次型$f(x)=x^\t A x$（因此假设$A$是对称矩阵），我们来求$f'(x)$. 为此，考虑一个新函数$g(x,y)=x^\t A y$，则$f(x)=g(x,x)$，于是

\begin{align*}
    f'(x)&=g'(x,x)\\
    &=\partial_1 g(x,x)\cdot\frac{\partial x}{\partial x}+\partial_2 g(x,x)\cdot\frac{\partial x}{\partial x}\\
    &=\partial_1 g(x,x)+\partial_2 g(x,x).
\end{align*}
\[\]

我们来计算$\partial_1 g(x,x)$，$g(x,y)=(Ay)^\t x$，根据\Cref{example:linear-derivative}，$\partial_1 g(x,y)=(Ay)^\t$，于是$\partial_1 g(x,x)=x^\t A^\t$，同理$\partial_2 g(x,x)=x^\t A$，因此$f'(x)=x^\t A+x^\t A^\t=(2Ax)^\t$.
\end{example}

\begin{remark}
在求向量对向量的导数的时候，很容易搞不清楚Jacobi矩阵的行列顺序，一个简单的检查方法是看看导数的维度是否正确，例如我们可以试试这一个矩阵是否可以乘自变量，然后得到因变量的维数，例如\Cref{example:quadratic-derivative} 中，如果我们求出来导数是$2Ax$，那么$2Axx$是一个无意义的量，说明我们的导数求错了，应该要进行转置. 
...
\begin{proposition}\label{proposition:partial-commute}
...
我们来看一个重要的例子. 
\begin{example}\label{example:multi-taylor}
...
再求二阶导数，根据\Cref{proposition:partial-commute} 和链式法则，
\begin{align*}
    g''(t)&=\sum_{i=1}^nh_i\frac{\d}{\d t}(\partial_i f(x+th))\\
    &=\sum_{i=1}^nh_i\sum_{j=1}^n\partial_j(\partial_i f(x+th))h_j\\
    &=\sum_{i,j=1}^nh_ih_j\partial_{j,i}f(x+th).
\end{align*}
用nabla算子，我们可以写作$g''(t)=(h^\t\nabla)^2f(x+th)$. 一般地，我们有
\[g^{(m)}(t)=\sum_{i_1,\dots,i_m}\partial_{i_m,\dots,i_1}f(x+th)h_{i_1}\dots h_{i_m}=(h^\t\nabla)^mf(x+th).\]
\end{example}
...
\[H_f(x)=\begin{pmatrix}
    \partial_{1,1}f(x) & \dots & \partial_{1,n}f(x) \\
    \vdots & \ddots & \vdots \\
    \partial_{n,1}f(x) & \dots & \partial_{n,n}f(x)
\end{pmatrix}.\]

显然，Hessian矩阵是一个对称矩阵，因而可以构成某个二次型. \Cref{example:multi-taylor} 中二阶导数其实已经暗示了这一点，我们可以将二阶导数写成一个二次型的形式：
\[g''(t)=h^\t H_f(x+th)h.\]

\subsection{微分学基本定理}

类似一元函数，我们讨论极值与导数的关系，我们也不给出具体证明. 注意，一元函数的极值的定义（\Cref{definition:extremum}）已经包含了多元函数情形，所以这里就不再重复. 
...
\begin{theorem}[Lagrange中值定理]\label{theorem:lagrange-mid-multi}
...
\begin{example}
考虑匀速圆周运动，$r(t)=(\cos t,\sin t)$，其速度向量是$r'(t)=(-\sin t,\cos t)$. 当绕一个周期之后，位置又回到了原点，于是$r(2\pi)-r(0)=0$，然而$r'(t)$恒不为$0$，因此不存在$\xi\in(0,2\pi)$使得$r(2\pi)-r(0)=r'(\xi)(2\pi-0)$.
\end{example}

不过，中值定理的弱化版本，有限增量定理，是成立的：

\begin{theorem}[Lagrange有限增量定理]\label{theorem:lagrange-finite-multi}
...
\begin{example}\label{example:lagrange-finite-multi}
这个例子探讨如何用二阶导数控制一阶导数的变化. 这一部分需要算子范数和谱理论的知识，请参阅\Cref{sec:operator-norm}. 

假设$f\in C^2(X)$，$X\subseteq\R^n$，那么对任意$x,y\in X$满足$[x,y]\in X$，根据\Cref{theorem:lagrange-finite-multi} 和Hessian矩阵的定义，我们有
\[\norm{\grad f(x)-\grad f(y)}\leq \norm{x-y}\sup_{\xi\in(x,y)}\norm{H_f(\xi)}.\]

我们讨论两种情况，首先假设$X$是紧集. 因为$H_f(x)$连续，根据\Cref{example:metric-norm-continuous}，$\norm{\cdot}$连续，再根据\Cref{proposition:continuous-composition}，$\norm{H_f(x)}$连续. 由于$X$是紧集，根据Weierstrass最值定理（\Cref{theorem:weierstrass}）$\norm{H_f(x)}$在$X$上取到最大值$M$，因此，我们有
\[\norm{\grad f(x)-\grad f(y)}\leq M\norm{x-y}.\]

这一推导表明紧集上的$C^2$函数的梯度是\textit{Lipschitz连续}的. 所谓$F$是Lipschitz连续的，指的是存在一个常数$M$，使得对于任意定义域里的$x,y$，我们都有$\norm{F(x)-F(y)}\leq M\norm{x-y}$. 

在第二种情况下，假设$X=\R^n$，我们使用$L^2$范数，于是根据\Cref{theorem:symmetric-operator-norm}，我们有
\[\norm{\grad f(x)-\grad f(y)}\leq\sup_{\lambda\in\sigma(H_f(z)),z\in(x,y)}|\lambda|\cdot \norm{x-y}.\]
这里$\sigma(A)$是矩阵$A$的谱. 

因此，只要知道了Hessian矩阵的谱，我们就可以控制梯度的变化，这一点对于凸优化算法非常重要，具体讨论见\Cref{example:gradient-descent}.
\end{example}

最后，我们要讨论高阶导数与极值的关系. 首先，利用\Cref{example:multi-taylor} 和一元的Taylor公式，我们可以得到多元函数的Taylor公式：
...
\begin{theorem}\label{theorem:second-derivative-test}
...
微积分中，还有一类非常重要的问题，那就是解方程，我们看一个非常简单的例子. 设$f:\R^2\to\R$，$f(x,y)=x^2+y^2-1$，我们来求解方程$f(x,y)=0$，也就是求单位圆的方程. 由于$f$是一个二次型，因此我们可以直接求出它的根：
    \[y=\pm\sqrt{1-x^2}.\]
比如，考虑圆周上的点$(0.6,0.8)$的附近，$x$就可以把$y$表示出来：$y=\sqrt{1-x^2}$. 另一方面，如果考虑点$(0.6,-0.8)$的附近，我们也可以写出$y=-\sqrt{1-x^2}$. 
...
如果$k<m$，那么线性映射$y\mapsto By$的秩是$k<m$，根据\Cref{corollary:kernel-image-isomorphism}，这个映射的核不是$\{0\}$，所以对于任意一个满足$Ax_0+By_0=c$的$(x_0,y_0)$来说，总可以再加上一个$y\neq 0$使得$Ax_0+B(y+y_0)=c$且$\norm{y}$充分小. 因此对任何$x_0$，都找不到一个$y$的邻域，其中有唯一的$y$使得$Ax_0+By=c$，所以我们也不能解出$y=y(x)$.
...
\begin{theorem}[隐函数定理]\label{theorem:implicit-function}
...
这个定理的证明需要一定的工作量，我们这里不给出.

隐函数定理的一个特例是向量值函数的反函数的存在性定理：

\begin{theorem}[反函数定理]\label{theorem:inverse-function}
...
\begin{example}[极坐标]\label{example:polar-coordinates}
    考虑一个半平面$\R_{\geq 0}\times\R=\{(r,\phi)\in\R\times\R:r\geq 0\}$. 我们将它映射到$\R^2$平面上，映射$f$定义为
