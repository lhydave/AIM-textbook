...
\begin{definition}[证据支持]
    假设 $X$ 和 $Y$ 是命题公式，实数$t\in [0.5, 1]$. 如果 $\Pr(Y|X)>t$, 我们称\emph{证据$X$支持$Y$的强度大于 $t$}. 
\end{definition}
...
现在我们正式陈述这一定理. 首先，让观测次数为无穷，即一列$X_1,X_2,\dots$. 此时，\emph{可交换性}指的是任意有限个$X_{i_1},X_{i_2},\dots,X_{i_n}$以及任意一个集合$\{i_1,\dots,i_n\}$上的排列$\pi$都有
\[
