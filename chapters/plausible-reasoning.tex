...
本章试图建立合情推理的一个数学模型，这一模型与Bayes概率论紧密地联系在一起. 我们会用它来研究一些有趣的推理谬误，包括合取谬误，并与演绎推理进行对比. 最后，我们会讨论Bayes概率论中先验为什么是存在的.
...
\begin{example}\label{example:semantic-implication}
...
完备性定理的一个直接推论是，我们可以用真值表来检查从前提出发是否能推出结论. 例如，在\Cref{example:semantic-implication} 中我们验证了$p\to q,q\to r\vDash p\to r$\footnote{在书写时，我们常常省略$\Gamma$中的花括号，因此这里写作$p\to q,q\to r\vDash p\to r$而不是$\{p\to q,q\to r\}\vDash p\to r$. 对于符号$\vdash$我们遵循同样的规则.}，所以也有
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    \item 利用似然的三个规则将似然$[(A \vee \neg B) \to (\neg C \wedge D)]  |  E$写为只含数字$1$和原子命题的合取的似然（形如$A|B$, $AB|C$等，但不能是$\bar{A}B|C$这种）的式子.
