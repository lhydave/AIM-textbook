...
\begin{example}[二维空间中的约束]\label{example:2d-constraint}
考虑二维空间中的如下约束：
\begin{align*}
...
\begin{example}[三维空间中的约束]\label{example:3d-constraint}
考虑三维空间中的如下约束：
\begin{align*}
...
我们可以从另一角度来理解这两个例子. 在\Cref{example:2d-constraint} 中，原本$(x_1,x_2)$两个维度都是自由选择的，所以我们可以用两个互相独立的参数来描述这个点. 当加入约束$h_1(x)=0$之后，给定一个$x_1$，我们我们并不能自由选择$x_2$，而是要满足约束$h_1(x)=0$. 容易看出，我们只用一个参数$\theta$就可以描述这个约束下的点：
\[
...
类似地，在\Cref{example:3d-constraint} 中，原本$(x_1,x_2,x_3)$可以用三个互相独立的参数来描述，当加入约束$h_1(x)=0$之后，我们只能用两个独立的参数来描述，而加入约束$h_2(x)=0$之后，我们只需要一个参数就可以描述这个约束下的点. 这对应的就是三维空间被约束到了二维空间，再被约束到了一维空间.

更一般地，如果$h:\R^n\to\R^m$，那么$h$的每一维都对$\R^n$增加了一个约束，最终$h(x)=0$定义了一个$n-m$维的曲面（在通常情况下）. 

不过，这一性质并不是绝对的，请看下面的例子. 

\begin{example}\label{example:3d-constraint-2}
在三维空间中，考虑如下约束：
\begin{align*}
    h_1(x)&=x_1^2+x_2^2+x_3^2-1=0,\\
    h_2(x)&=x_1-1=0.
...
\Cref{example:3d-constraint-2} 的第一个约束为什么不符合预期？在点$(1,0,0)$，球面$h_1(x)=0$的切平面恰好是约束$h_2(x)=x_1 - 1 = 0$，这意味着在这个点，可行解对应的空间“降维”了！这说明，“切平面”这样的概念对于维数有着至关重要的作用. 
...
\begin{example}[三维球面的切空间]\label{example:tan-space}
    考虑三维空间中的单位球面
    ...
    \[\dot{x}_\theta(0)=(0,\cos\theta,\sin\theta),\]
    所以，$x^*$处的切空间至少包含以下集合
    \begin{align*}
        \{(0,k\cos \theta,k\sin\theta):k\in\R,\theta\in[0,2\pi)\}= \{(0,y,z):y,z\in\R\}.
    \end{align*}
    因为球面是一个二维曲面，所以切空间不可能是整个三维空间. 因此，过$x^*$的切空间就是
    \[T_{x^*}(S^2)=\{(0,y,z):y,z\in\R\}.\]
\end{example}

\Cref{example:tan-space} 中的切空间是一个二维的线性空间. 直观上，任何切空间都是应该一个线性空间，这也是它名字的来源. 
...
我们已经证明$T_{x^*}(S)\subseteq M^\perp$. 进一步，\Cref{example:tan-space} 的结果表明，$T_{x^*}(S)=M^\perp$，即切空间和$M^\perp$是相等的. 然而，如果对于\Cref{example:3d-constraint-2} 中的第一个$h$，我们会发现切空间和$M^\perp$是不相等的：$h(x)=0$对应的是单个点，对于单个点的切空间自然是一个零维空间，然而，和$\nabla h(x^*)$正交的空间是整个二维空间！

以上例子说明两件事，首先，切空间和$M^\perp$不一定相等；其次，切空间和$M^\perp$的关系和曲面的维数有关. 为了说明这一点，我们引入\textit{正规点}的概念. 

\begin{definition}[正规点]\label{definition:regular-point}
...
\begin{theorem}[正规点切空间刻画定理]\label{theorem:tan-space-characterize}
...
\begin{theorem}[条件极值的一阶必要条件]\label{theorem:eq-opt-cond-1}
...
这一个求解条件极值的方法会在大部分微积分课程中给出，我们这里的更重要的任务是给出这一方法的几何解释. 注意，\Cref{theorem:eq-opt-cond-1} 本质上在说，$\nabla f(x^*)$是$\nabla h_i(x^*)$的线性组合，所以我们的目标就是得到这一事实. 
...
现在，回忆\Cref{theorem:tan-space-characterize}，我们知道切空间$T_{x^*}(S)=M^\perp$，因此
\[\nabla f(x^*)^\t\in (M^\perp)^\perp=M=\left\{\sum_{i} \lambda_i \nabla h_i(x^*)^\t:\lambda_i\in\R\right\}.\]
换言之，$\nabla f(x^*)$是$\nabla h_i(x^*)$的线性组合，这就证明了\Cref{theorem:eq-opt-cond-1}.


最后，作为应用，我们考虑一个例子.

\begin{example}[最大熵]\label{example:max-entropy}
...
\begin{theorem}[Karush-Kuhn-Tucker条件，\cite{karushMinimaFunctionsSeveral1939,kuhnNonlinearProgramming1951}]\label{theorem:KKT}
...
\end{align}
自然，$x^*$需要满足可行解的条件，即$h(x^*)=0$，$g(x^*)\leq 0$，$x^*\in\Omega$.
\end{theorem}

\begin{proof}
考虑$x^*$的邻域$U$，在这个邻域内，我们可以将问题 \eqref{eq:ineq-constraint-inequality-differentiable} 写作 \eqref{eq:ineq-constraint-inequality-differentiable-active}，即只考虑激活的约束. 由于$x^*$是一个极小值点，根据\Cref{theorem:eq-opt-cond-1}，存在$\lambda\in\R^m,\mu\in\R^{|J|}$使得
\[
\nabla f(x^*)+\lambda^\t \nabla {h(x^*)}+\mu_J^\t \nabla g_J(x^*)=0.
\]
...
下面我们说明为什么这样的$y$存在. 因为$x^*$是整个优化问题的正规点，所以$S$中的约束是线性无关的，因而$x^*$也是去掉了约束$g_k\leq 0$之后的正规点. 根据\Cref{theorem:tan-space-characterize}，$T_{x^*}(S)=M^\perp$，其中
...
\begin{proposition}[互补松弛条件]\label{proposition:complementary-slackness}
...
我们在推导KKT条件（\Cref{theorem:KKT}）的时候，最终得到了如下的Lagrange函数：
...
\begin{proposition}\label{proposition:lagrange-primal}
...
\begin{theorem}[弱对偶定理]\label{theorem:weak-duality}
    对于任意优化问题 \eqref{eq:non-differentiable-constraint}，我们有
    \[d^*\leq p^*.\]
    因此，对任意原始规划的可行解$x$和对偶规划的可行解$(\lambda,\mu)$，我们有
    \[\phi(\lambda,\mu)\leq\omega(x).\]
\end{theorem}
...
\begin{proof}
    ...
    注意到对任意原始规划的可行解$x$，成立$\omega(x)\geq p^*$；对任意对偶规划的可行解$(\lambda,\mu)$，成立$\phi(\lambda,\mu)\leq d^*$. 因此，我们有
    \[\phi(\lambda,\mu)\leq \omega(x).\]
    这就完成了证明.
\end{proof}

\subsection{Slater条件，Farkas引理，强对偶定理}
...
\end{lemma}
\begin{proof}
    这一证明非常类似\Cref{theorem:convex-epi} 的证明. 考虑系数$\theta\in[0,1]$和两个点$(t_1,u_1^\t,v_1^\t)^\t,(t_2,u_2^\t,v_2^\t)^\t\in\mathcal G^*$，我们要证明
    \begin{equation}
...
\begin{theorem}[强对偶定理]\label{theorem:strong-duality}
...
\begin{proof}
    ...
    设$h(x)=Ax+b$，我们不妨设$A$是满秩矩阵，否则约束$h(x)=0$要么无法满足，要么有冗余的约束. 当有冗余约束的时候，冗余约束可以用其他约束线性表示出来，因此后面的讨论总可以先把冗余约束去掉，然后再进行线性组合表达出原先的冗余约束.

    根据\cref{lemma:G*-convex}，$\mathcal G^*$是凸集. 我们需要选出来从$K$作出的切平面. 一个自然的选择是使用分离超平面定理（\Cref{theorem:separation-hyperplane}）. 定义另一个凸集为
    \[\mathcal{C}=\{(t,0,0)^\t: t<p^*\}.\]
    ...
    因此，我们可以用分离超平面定理（\Cref{theorem:separation-hyperplane}）找到一个非零$\alpha=(\alpha_t,\alpha_u^\t,\alpha_v^\t)^\t$和$\beta$使得
    \begin{align*}
...
除了Slater条件，我们还有一个条件也能保证强对偶定理成立，那就是线性规划，尽管线性规划不一定满足Slater条件，但是只要线性规划有可行解，它就成立强对偶定理. 线性规划的强对偶定理依赖于完全不同的几何直观，即\textit{Farkas引理}. 

\begin{theorem}[Farkas引理]\label{theorem:farkas}
    设$A$是一个$m\times n$的实矩阵，$b\in\R^m$，那么下列命题之一恰好成立：
    \begin{itemize}
        \item 存在$x$使得$Ax=b$且$x\geq 0$.
        \item 存在$p$使得$A^\t p\geq 0$且$p^\t b<0$.
    \end{itemize}
\end{theorem}

\begin{proof}
我们给一个几何证明，这一证明同时也解释了Farkas引理的几何直观. 设矩阵$A$的列为$A_1,\dots,A_n$，那么$Ax=\sum_i x_iA_i$，因而这是由$A_i$线性组合而成的一个新向量. 这些向量的非负组合形成一个锥$\mathsf{Cone}(A) = \left\{Ax: x\geq 0\right\}$.

考察向量$b$，它只会有两种情况：
\begin{itemize}
    \item $b$在锥的内部，或者说，$b$可以被表示为$Ax$的形式，因而存在$x$使得$Ax=b$且$x\geq 0$（第一种情况）.
    \item $b$在锥的外部，或者说，$b$不能被表示为$Ax$的形式. 此时，线段$B=\{\lambda b:\lambda\in[0,1]\}$和$\mathsf{Cone}(A)$都是非空凸集，仅有一个公共点，是二者的边界点. 因而根据\Cref{theorem:separation-hyperplane}，存在一个超平面将它们分开. 具体来说，设这个超平面的法向量为$p$（非零），那么，
    \begin{itemize}
        \item 对任意$Ax\in\mathsf{Cone}(A)$，$p^\t (Ax)\geq 0$，特别地，对$i=1,\dots,n$，$p^\t A_i\geq 0$.
        \item 对任意$\lambda\in[0,1]$，$p^\t (\lambda b)\leq 0$，特别地，$p^\t b<0$.
    \end{itemize}
\end{itemize}
\end{proof}

利用Farkas引理，我们可以证明线性规划的强对偶定理. 
\begin{theorem}[线性规划的强对偶定理]\label{theorem:strong-duality-linear}
    设原始规划为
    \begin{align*}
    ...
    对偶规划为
    \begin{align*}
        \maximize_{y}\quad&b^\t y\\
        \text{s.t.}\quad&A^\t y=c,\\
        &y\geq 0.
    \end{align*}
如果原始规划和对偶规划都有可行解，那么原始规划的最优值等于对偶规划的最优值.
\end{theorem}
\begin{proof}
    设原始规划和对偶规划的可行解分别是$\bar{x}$和$\bar{y}$，那么根据弱对偶定理，$b^\t \bar{y}\leq c^\t \bar{x}$. 所以原始规划和对偶规划的值都是有界的. 设原始规划的最优值为$p^*$，最优解为$x^*$. 

    设$a_i$为$A$的第$i$行形成的向量（一个列向量），$I=\{i: a_i^\t x^*=b_i\}$为激活的约束下标集合. 首先证明，如果向量$d$满足$a_i^\t d\geq 0$，$i\in I$，那么，$c^\t d\geq 0$. 给定一个充分小的正数$\epsilon$，我们有
    \[a_i^\t (x^*+\epsilon d)\geq a_i^\t x^*=b_i,\quad i\in I.\]
    假设$\epsilon$充分小，那么，对于所有$i\notin I$，我们有$a_i^\t (x^*+\epsilon d)>b_i$，因此，$x^*+\epsilon d$是原始规划的可行解. 由于$x^*$是原始规划的最优解，所以
    \[c^\t (x^*+\epsilon d)\geq c^\t x^*\implies c^\t d\geq 0.\]
    由Farkas引理，第二种情况不成立，因而第一种情况成立，即存在非负$y_i^*$（$i\in I$）使得
    \[c=\sum_{i\in I} y_i^* a_i.\]
    补充定义$y_i^*=0$（$i\notin I$），那么
    \[c=A^\t y^*,\quad y^*\geq 0.\]
    因而，$y^*$是对偶规划的可行解. 此外，
    \[b^\t y^*=\sum_{i\in I} y_i^* b_i=\sum_{i\in I} y_i^* a_i^\t x^*=c^\t x^*=p^*.\]
    而弱对偶定理表明，对对偶规划的任意可行解$y$，$b^\t y\leq p^*=b^\t y^*$. 因而，$y^*$是对偶规划的最优解，并且强对偶定理成立.
\end{proof}


至此，我们已经学习了约束优化极值的多种性质，现总结如下：
\begin{itemize}
    \item \textbf{Lagrange 乘子法}（\Cref{theorem:eq-opt-cond-1}）：适用于仅含等式约束的光滑优化问题，给出了极值的一阶必要条件. 
    \item \textbf{KKT 条件}（\Cref{theorem:KKT}）：适用于同时包含等式和不等式约束的光滑优化问题，给出了极值的一阶必要条件. 
    \item \textbf{松弛互补条件}（\Cref{proposition:complementary-slackness}）：适用于包含等式和不等式约束的优化问题，描述了不等式约束对应的 Lagrange 乘子的符号性质. 
    \item \textbf{弱对偶定理}（\Cref{theorem:weak-duality}）：对于任意优化问题，原始问题的最优值总是不小于对偶问题的最优值. 
    \item \textbf{强对偶定理}（\Cref{theorem:strong-duality}）：对于满足 Slater 条件的凸优化问题（或线性规划问题），原始问题的最优值等于对偶问题的最优值. 
\end{itemize}
...
\section{应用：支持向量机（SVM）}\label{sec:svm}
...
\begin{enumerate}[wide, labelindent=0pt]
    \item 用Lagrange乘子法重新证明\Cref{proposition:entropy-maximum}.
    ...
    \begin{enumerate}
        ...
        \item 假设$f$是$\R^n$上的可微凸函数，$Q$是一个闭凸集. 证明：$x^*$是$f$在$Q$上的最小值点当且仅当对任意$x\in Q$，$\langle f'(x^*),x-x^*\rangle\geq 0$.
    \end{enumerate}
