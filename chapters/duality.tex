...
设原始规划对应的最低点为
    \[K=(f(x^*),h(x^*)^\t,g(x^*)^\t)^\t=(p^*,u^*,v^*)^\t,\]
从\Cref{fig:weak-duality} 看，如果$\mathcal G$完全位于过$K$的（水平）切平面上方并且$\mathcal G$在$t$轴左侧不为空，那么$p^*$和$d^*$一定是相等的，此时我们称之为\emph{强对偶定理}. 我们可以把这一条件形式化为\emph{凸规划}和\emph{Slater条件}.
...
用Lagrange乘子法重新证明\Cref{prop:entropy-maximum}.
