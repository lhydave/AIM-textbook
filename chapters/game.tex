...
\begin{itemize}
    ...
    \item 完全信息与非完全信息：尽管这是一个直观的概念，但是如何在数学上区分完全信息与非完全信息却是极其困难的，我们这里给一种思路. 正如我们也没有数学上定义什么是博弈一样，考虑到博弈的多样性，我们也不会尝试给一般情况从数学上定义完全或者非完全信息. 
    
    我们将博弈本身也看成一个玩家\footnote{通常，在扩展式博弈中，我们将它称之为“天”（nature）. 这里借用了中国传统文化的观念，“天”常被视为一种至高无上的力量或存在，例如“天命”和“无法无天”. }，那么，完全信息意味着，任何玩家不依赖其他玩家，根据自己所拥有的信息，可以（非确定地）模拟出整个博弈的进行过程. 换句话说，他可以“扮演”其他任何角色. 反之，非完全信息意味着，玩家不能模拟博弈，这实际上意味着他无法获取所有需要的信息来进行模拟，特别地，他没有其他玩家的私有信息.

    井字棋是一个完全信息博弈. 在博弈的任何时刻，所有玩家都清楚地知道棋盘的当前状态、所有已经发生的行动以及游戏规则. 任何一个玩家都可以设想自己是对手，并基于当前局面推演对手所有可能的后续行动和自己的应对，从而模拟整个博弈的后续进程. 玩家做出决策所需要的所有信息都是公开的.

    斗地主是一个非完全信息博弈. 每个玩家只知道自己的手牌，而不知道其他玩家的手牌（这是私有信息）. 因此，一个玩家无法准确模拟其他玩家的决策过程或整个博弈的精确走向，因为他缺乏对其他玩家手牌信息的了解.
...
\begin{theorem}[Zermelo定理]\label{theorem:zermelo}
...
从博弈树的叶节点往根节点推理，见图\Cref{fig:game-tree} 的收益部分，这个收益表示从这个节点出发，不论对手如何行动，玩家X总有一种策略能够拿到的收益. 
...
\begin{theorem}[有平局的Zermelo定理]\label{theorem:zermelo-draw}
...
究其原因，人的思考以及机器搜索的过程实际上是前向探索的过程. 如何进行（启发式）搜索是取得胜利重要的因素. 在本章开头，我们讲述了AlphaGo的故事，这正是一个很好的例子. 下面我们就如何对围棋进行建模进行讨论.
...
\begin{itemize}
    \item 他可以按照他的技术水平（即启发式搜索函数）在脑中模拟一场围棋比赛，并且假定对手和自己一样聪明. 最终，这一模拟会有一个结果（输或者赢），这一结果反映了他的策略的好坏. 这就是用MCTS自博弈的过程.
...
MCTS的过程较为复杂，我们单独介绍. 树的组成如下：搜索节点是状态$s$，边是状态-行动对$(s,a)$. 每条边需要存储以下信息：
\begin{itemize}
    \item $N(s,a)$：边的访问次数.
...
接下来，MCTS要做如下迭代（见\Cref{fig:mcts}）：
...
\begin{example}[囚徒困境]\label{example:prisoner}
...
\begin{example}[猜硬币游戏]\label{example:matching-pennies}
...
回忆\Cref{example:prisoner}，它有如下收益矩阵：
\[
\begin{pmatrix}
-1,-1&-10,0\\
0,-10&-5,-5\\
...
\begin{theorem}\label{theorem:zero-sum-pure-NE}
...
\begin{itemize}
    ...
    \item 为什么$V$是对数的形式？在\Cref{chap:information-theory}中我们解释过，这样形式的收益函数是交叉熵损失函数的形式. 因此这一损失函数可以衡量两个分布之间的相似性，对于随机的数据来说，这是一个比较好的衡量方式.
\end{itemize}
...
因此，我们需要计算玩家的期望效用. 为此，引入混合\textit{策略组合}：$\sigma=(\sigma_i)_{i\in I}$，其中$\sigma_i\in\Delta(A_i)$，$\sigma_i(a_i)$表示$a_i$在$\sigma_i$中的概率. $\sigma$是一个$(A_i)_{i\in I}$上的概率分布，每一维相互独立. 当所有玩家选定策略之后，玩家$i$的期望收益是：
...
\begin{theorem}[Nash均衡存在性定理，\cite{nashEquilibriumPointsNperson1950,nashNonCooperativeGames1951}]\label{theorem:Nash-existence}
...
\begin{theorem}[无差别原理]\label{theorem:indifference-principle}
...
本节我们将讲述一种动态博弈和正则形式博弈的结合：随机博弈. 为了引入随机博弈，我们需要Markov链和Markov决策过程（MDP）相关的知识，更详细的讨论请参阅\Cref{chap:markov-chain}. 一个MDP有如下的组成：
\begin{itemize}
\item 有限状态集合：$\calS=\{s_1,s_2,\dots,s_N\}$.
\item 有限动作集合：$\mathcal A$.
\item 每个状态具有自己的动作空间：$\mathcal A_s = \{a_{s,1},a_{s,2},\dots, a_{s,N}\}, s\in S$.
\item 每个动作空间有限：$|\mathcal A_s|=n_s, s\in S$.
...
\begin{theorem}[Bellman方程]\label{theorem:stochastic-game-bellman}
...
\begin{definition}[Markov最优反应]
    对于一个随机博弈，给定其他玩家的平稳策略$\boldsymbol \pi_{-i}$，玩家 $i$ 的\textbf{Markov最优反应}是一个平稳策略 $\boldsymbol \pi_i$，使得对于任意状态 $s$ 和任意平稳策略$\boldsymbol \pi_i'$，
...
\begin{theorem}[MPE存在性定理]\label{theorem:MPE-existence}
对于$N$个玩家、有限状态、有限动作空间的随机博弈，MPE存在.
\end{theorem}

下面我们介绍Shapley~\cite{shapleyStochasticGames1953} 关于双人零和随机博弈情形的证明，这一证明基于Banach不动点定理（\Cref{theorem:banach-fixed-point}），非常类似MDP最优解存在性的证明（见\Cref{chap:fixed-point-theory}的习题 \ref{exercise:MDP-fixed-point}），但多了一些零和博弈的性质. 对于一般的情况，可以类似Nash均衡存在性，利用Brouwer不动点定理证明，见习题 \ref{exercise:proof-MPE-existence}.
...
利用多智能体强化学习，我们可以训练出极其强大的人工智能，DeepMind在星际争霸2中训练出的AlphaStar~\cite{vinyalsGrandmasterLevelStarCraft2019} 就是一个例子. 通过人类数据与多智能体强化学习的自我训练，AlphaStar在星际争霸2的比赛中击败了世界冠军. 它展示了博弈论如何为人工智能的发展提供语言和训练方法. 
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    \item \label{exercise:zermelo-draw} 证明\Cref{theorem:zermelo-draw}.
    ...
    \begin{enumerate}
        ...
        \item 证明：$G$存在纯策略Nash均衡当且仅当其中一个玩家存在必胜策略，从而证明\Cref{theorem:zero-sum-pure-NE}.
    \end{enumerate}

    \item \label{exercise:proof-Nash-existence} 使用Brouwer不动点定理证明\Cref{theorem:Nash-existence}.
        \begin{hint}
        考虑混合策略空间$S$上的函数$f(\sigma) = \sigma'$的不动点，其中
        \[\sigma'_i(a_i) = \frac{\sigma_i(a_i) + \phi_{i,a_i}(\sigma)}{1+\sum_{b_i\in A_i}\phi_{i,b_i}(\sigma)},\]
        这里$\phi_{i,a_i}(\sigma)=\max\{0, u_i(a_i,\sigma_{-i})-u_i(\sigma_i,\sigma_{-i})\}$, $a_i$是玩家$i$的一个行动. 
        \end{hint}
    ...
    \begin{enumerate}
        \item 证明\Cref{theorem:indifference-principle}.
    ...
    \item \label{exercise:stochastic-game-bellman} 证明\Cref{theorem:stochastic-game-bellman}.
    
    \item *\label{exercise:proof-MPE-existence} （\cite{dengComplexityComputingMarkov2023}）仿照习题 \ref{exercise:proof-Nash-existence}，使用Brouwer不动点定理证明\Cref{theorem:MPE-existence}.
\end{enumerate}