...
下面我们介绍Shapley~\cite{shapleyStochasticGames1953} 关于双人零和随机博弈情形的证明，这一证明基于Banach不动点定理（\Cref{thm:banach-fixed-point}），非常类似MDP最优解存在性的证明（见\Cref{chap:fixed-point-theory}的习题 \ref{exercise:MDP-fixed-point}），但多了一些零和博弈的性质. 对于一般的情况，可以类似Nash均衡存在性，利用Brouwer不动点定理证明，见习题 \ref{exercise:proof-MPE-existence}.
