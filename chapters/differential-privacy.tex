...
直到2024年，Netflix也再也没有举办过第二届 Netflix Prize 竞赛，它的服务也已经转变到互联网上的在线视频为主. 然而，在当今世界，数据隐私的问题反而变得愈发严重. 我们每天看的在线视频、购物、社交网络等行为都会产生大量的数据，根据Statista在2020年的一项估计，2024年每人每天产生的数据量会达到63.5GB!然而，这些数据都被私人公司所掌握！
...
本章开篇的Netflix Prize案例展示了一个典型的数据隐私问题. 实际上，不仅资本需要数据，科研也极其需要数据. 以医学为例，我们需要大量真实的病人提供病情数据. 但这些数据可能都涉及到病人的隐私信息，例如一些\textit{敏感数据}. 因此，我们必须找到一种方法，既可以收集到这些数据，又保护病人的隐私信息.
...
考虑数据的空间$\mathcal X$，其中的每一个元素都包含了个体所有可能的属性，例如姓名、性别、年龄、国籍等. 考虑$n$个人的数据，形成了有序的数据集
\[x = (x_1, \dots, x_n) \in \mathcal X^n.\]  

我们希望对这些数据进行一定的加工，例如用这些数据来训练一个机器学习模型. 抽象来说，加工的过程可以被看成一个随机算法$A$，其定义如下：
\begin{definition}[随机算法]
    考虑输入空间$\mathcal X$和输出空间$\mathcal Y$，\textbf{随机算法}$A$被定义为随机映射 $A : \mathcal X \to \mathcal Y$，即固定输入 $x \in \mathcal X$下，$A(x)$ 是输出空间 $\mathcal Y$ 上的一个随机变量. 等价地，我们可以把 $A(x)$ 看成是输出空间$\mathcal Y$上的一个概率分布 $P_x$. 
\end{definition}

例如，大语言模型本身就是一个随机算法，模型输入是一系列的文本，模型输出是下一个词元的概率分布（见\Cref{sec:language-models}），如果对这一分布采样，我们就得到了下一个词元.

当我们替换一个人的数据时，我们希望结果分布的变化可以控制在一定范围内. 为此，我们引入相邻数据集的概念：

\begin{definition}[$k$-相邻数据集]
    设 $x, x' \in \mathcal X^n$，如果 $x$ 和 $x'$ 最多有$k$条数据不同，即至多存在$k$个不同的$i_1,\dots,i_k \in [n]$ 使得 $x_{i_j}\neq x_{i_j}'$对$j\in[k]$成立，那么称 $x$ 和 $x'$ 是 \textbf{$k$-相邻的}. 
...
考虑非负实数$\epsilon$和随机算法 $A : \mathcal X^n \to \mathcal Y$，如果对于任意一对$1$-相邻数据集$x, x'$，对任意（可测）值集 $E \subseteq \mathcal Y$，有
\[
...
对于离散型的输出，我们有如下等价定义：
\begin{proposition}\label{proposition:discrete-dp}
...
对连续型的输出，我们有如下等价定义：
\begin{proposition}\label{proposition:continuous-dp}
...
\begin{proposition}[复合性，两个算法的情形]\label{proposition:composition}
...
\begin{proposition}[复合性，多个算法的情形]\label{proposition:composition-multi}
    ...
    依次运行算法 $A_i$ 得到算法$A : \mathcal X^n \to \mathcal Y_1 \times \dots \times \mathcal Y_k$，那么$A$是$\epsilon$-DP，其中$\epsilon = \sum_{i=1}^k \epsilon_i$.
\end{proposition}
\begin{proof}
    对$k$用数学归纳法. 
    \begin{itemize}
        \item 当$k=1$时，结论显然成立.
        \item 假设结论对$k$成立，我们要证明对$k+1$结论成立.
        
        考虑前$k$个算法的复合算法
        \[A'(x) = (A_1(x), A_2(A_1(x), x), \dots, A_k(A_{k-1}(\dots A_1(x), \dots), x)).\]
        由归纳假设，$A'$是$\sum_{i=1}^k\epsilon_i$-DP算法.

        由于$A_{k+1}$是$\epsilon_{k+1}$-DP算法，$A'$与$A_{k+1}$的复合算法是$A$，所以利用\Cref{proposition:composition}，$A$是$\sum_{i=1}^{k+1}\epsilon_i$-DP算法.
    \end{itemize}
\end{proof}

接下来，我们说明差分隐私最关键的性质：一旦被差分隐私算法处理过，无论后续如何处理，都不会影响隐私保护的效果.

\begin{proposition}[后处理]\label{proposition:post-processing}
...
\begin{proposition}[群体隐私]\label{proposition:group-privacy}
...
我们称这一算法为\textit{随机反应}（RR）算法，它在1965年被S. L. Warner提出~\cite{warnerRandomizedResponseSurvey1965}、后在1969年被Greenberg与他的合作者改造~\cite{greenbergUnrelatedQuestionRandomized1969}（远远早于差分隐私概念的提出！），被广泛用于调查敏感问题.
...
所以，对于一对$1$-相邻数据集$x, x'$和任意的$y \in \mathcal Y$，假设$x_j \neq x'_j$，不论$y_j$，$x_j$和$x'_j$的取值是什么，我们都有
    ...
    \end{align*}
由\Cref{proposition:discrete-dp}，RR算法是$\log 2$-DP算法.
\end{proof}
...
\begin{theorem}\label{theorem:laplace-mechanism}
...
因此，对于任意的$y \in \mathcal Y$，有
    ...
    \end{aligned}
    \]
根据\Cref{proposition:continuous-dp}，命题得证. 
\end{proof}

\subsection{DP版本Lloyd算法}
作为一个Laplace机制的具体实例，我们将$k$-均值聚类问题的经典算法Lloyd算法改造成一个差分隐私算法，这一算法首先是由 \cite{suDifferentiallyPrivateKMeans2016} 提出的.
...
注意，如果初始化的中心就是各个质心，那么Lloyd算法就已经找到最优解了. 事实上，可以证明，Lloyd算法最终会收敛到最优解. 

尽管Lloyd算法可以达到很好的效果，但它并不能保证DP性质. 我们希望对这一算法进行小规模的修改，让它具有$\epsilon$-DP的性质. 我们给出如下的DP版本Lloyd算法.
...
简单来说，我们对原来Lloyd算法中产生的中间值都是用Laplace机制来加噪声，最终得到的中心也是加了噪声的. 以下定理表明上面的算法确实是一个$\epsilon$-DP算法. 
\begin{theorem}\label{theorem:lloyd}
...
首先，作为一个迭代算法，我们可以把每轮迭代看成一次算法$A_t$的执行，而整个算法是这些算法的复合，整个算法一共有$T$次复合. 由\Cref{proposition:composition-multi}，我们只需要证明每一轮迭代是$(\epsilon/T)$-DP算法，那么整个算法就是$\epsilon$-DP算法.
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    \item \label{exercise:laplace-mechanism-multidimensional} 现考虑输出为$d$维的算法$f: \mathcal X^n \to  \R^d$，定义其全局灵敏度为
    \begin{align*}
    ...
    \item \label{exercise:gs-multidimensional} 设输出为$d$维的算法$f( x) = (\sum_{i=1}^n \Phi_1( x_i), \dots, \sum_{i=1}^n \Phi_d( x_i))$， $x_i\in\mathcal X$，满足$\Phi_j(\mathcal X)=\{0, 1\}$以及$\sum_{j=1}^d \Phi_j( x_i) = 1$. 请计算$\text{GS}_f$. 

    \item \label{exercise:lloyd} 证明\Cref{theorem:lloyd}.
    \begin{hint}
        利用第 \ref{exercise:laplace-mechanism-multidimensional} 题和第 \ref{exercise:gs-multidimensional} 题的结果.
    \end{hint}
    ...
    \begin{itemize}
        ...
        \[
            D_{\KL}(P_{\mathcal Y|\mathcal X^n=x}\| P_{\mathcal Y|\mathcal X^n=\tilde{x}}) \leq \epsilon.
        \]
    \end{itemize}
    ...
    \begin{hint}
        条件互信息可以写成K-L散度的形式：
            \[
                I(\mathcal X_{i};\mathcal Y|\mathcal X_{-i}) = \E \left[D_{\KL}\left(P(\mathcal Y|\mathcal X^{n}=\bar{x}^n)\Vert P(\mathcal Y|\mathcal X_{-i}=\bar{x}_{-i})\right)\right],
            \]
