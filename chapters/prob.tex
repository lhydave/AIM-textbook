...
注意到
\begin{align*}
    \Pr(C) = \Pr(C\cap B) &= \Pr\left(C\cap\bigcup_{i=1}^\infty A_i\right)\\
...
设$(\Omega,\scF,\Pr)$是一个概率空间，$X:\Omega\to\R$是一个函数. 如果对任意的$x\in\R$，都有$\{X\in\scB(\R)\}\in\scF$，则称$X$是一个\textbf{随机变量}. 
...
\[
    \Pr\left(\prod_{i=1}^n(a_i,b_i]\right) = \Delta_{a_1b_1}\Delta_{a_2b_2}\dots\Delta_{a_nb_n}F(x_1,x_2,\ldots,x_n).
\]
\end{theorem}
