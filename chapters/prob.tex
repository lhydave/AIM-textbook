...
\Cref{thm:lebesgue-differentiation-theorem} 意味着，绝对连续测度对应的分布函数几乎处处可以求导，并且所得到的导函数积分回去还是原来的分布函数，也就是微积分基本定理成立. 这样的函数我们称之为\textit{绝对连续函数}.
...
对照\Cref{thm:lebesgue-differentiation-theorem}，我们知道$p_{X_1,X_2}/p_{X_2}$具有密度函数的形式，所以连续性随机向量所定义的条件分布也是连续型分布，密度函数被$p_{X_1,X_2}/p_{X_2}$通常记作$p_{X_1|X_2}$，称为$X_1$在给定$X_2$的条件下的\textit{条件密度}. 
...
第三步，定义\textit{非负随机变量}的数学期望. 非负随机变量是指$X(\omega)\geq 0$对任意$\omega$成立的随机变量$X$. 考虑一列简单随机变量$\{X_n\}_{n=1}^\infty$，它满足对于每一个$\omega\in\Omega$都有当$n\to\infty$时$X_n(\omega)\uparrow X(\omega)$. 容易验证，$\E[X_n]$也是单调递增的，所以根据\Cref{prop:monotone-bounded}，$\E[X_n]$有有限的极限或者趋于正无穷，我们都记为$\lim_{n\to\infty} \E[X_n]$.
...
\begin{remark}
    在\Cref{prop:covariance-inner-product} 中，我们使用了$\Pr(X=C)=1$这样的表达. 在概率论中，如果一个事件是概率$1$发生的，我们称之为\textit{几乎必然}发生. 在涉及与数学期望有关的性质的时候，我们通常只能在几乎必然的意义下成立，而不能在一般意义下成立. 比如说，“在差一个常数的意义下，协方差是一个随机变量空间的内积”这句话其实并不准确，严格来说，应该是“在差一个常数和几乎必然相等的意义下，协方差是一个随机变量空间的内积”. 也就是说，如果$\norm{X}=0$，那么$X$几乎必然为常数.
\end{remark}
...
\begin{theorem}[Khinchin大数定律]\label{thm:khinchin}
...
\begin{proof}
    ...
    \end{align*}
    根据\Cref{thm:characteristic-function-moment}，我们有
    \[f_{X_1}(t)=1+it\mu+o(t),\quad t\to 0.\]
    于是
    \[f_{S_n}(t)=\left(1+\frac{it\mu}{n}+o\left(\frac{t}{n}\right)\right)^n\to\e^{it\mu},\quad n\to\infty.\]
    因此，根据\Cref{prop:characteristic-function-uniqueness}，$S_n$的分布趋于一个在$\mu$处集中的退化分布，根据定义，这意味着对任意$x$，当$n\to\infty$时，
