...
朴素的概率论通常讨论两种极端的情况，一个是可以用数数的方式来计算概率的情况，比如说掷骰子，另一个是用面积的方式来计算概率的情况，比如在随机选一个圆周上的点. 这两个情况分别对应了\textit{古典概型}和\textit{几何概型}. 

我们先给一些术语. 考虑一个随机试验，它的所有可能结果组成的集合称为\textbf{样本空间}，记为$\Omega$. 样本空间的元素称为\textbf{样本点}，通常记为$\omega$. 样本空间的\textit{某些}子集被称为\textbf{事件}. 我们来看看这些概念在朴素的概率论中都具体是什么. 
...
事件$A = [0, \pi)$表示选出了上半圆周，而事件$B = [0, \pi/2)\cup[\pi, 3\pi/2)$表示选出了右上或左下的$1/4$圆周. 
\end{example}
...
朴素的直觉告诉我们，选到上半圆周的概率是$1/2$，因为上半圆周刚好占了半个圆周. 所以几何概型的概率定义利用了\textit{体积}的概念. 事件$A$的概率定义为
\[
...
首先注意到，概率应该是一个函数，它的值域是$[0,1]$. 那么，它的定义域应该是什么呢？我们已经看到，概率应该定义在\textit{事件}上，而非\textit{样本点}上. 那么，概率可以定义在\textit{任意}事件上吗？这个问题很微妙，我们不在这里讨论. 这里只是指出，我们关心的并不总是任意事件，而是一类被$\sigma$-代数所刻画的事件. 
...
\subsection{条件概率，独立性}
接下来，我们讨论条件概率与独立性. 我们还是看掷两个骰子的例子. 掷完第一个骰子，我们马上观察结果，然后再掷第二个骰子. 问第一个骰子是$i$，第二个是$j$的概率是多少？如果继续套用原来的概率空间，很快就会觉得不对劲. 此时，第一个骰子的结果完全没有随机性！所以朴素的直觉告诉我们，这里的概率应该有另一个依赖于第一次投骰子结果的定义，这样的概率就是\textit{条件概率}. 
...
\begin{align*}
    \Pr(B|A)=\Pr(B)&\iff \frac{\Pr(A\cap B)}{\Pr(A)}=\Pr(B)\\
...
我们在定义中还给出了多个事件相互独立的定义，这一定义是说不管挑出其中多少有限个事件，他们都应该满足交的概率等于概率的积. 这并\textit{不等价}于任意两个事件都相互独立，我们看下面的例子. 
...
容易算出，
\begin{align*}
    \Pr(A\cap B)&=\Pr(A)\Pr(B)=1/9,\\
    \Pr(B\cap C)&=\Pr(B)\Pr(C)=1/9,
\end{align*}
所以$A,B,C$两两独立. 但是$A,B,C$不是相互独立的：
\[\Pr(A\cap B\cap C)=1/9\neq 1/27=\Pr(A)\Pr(B)\Pr(C).\]
\end{example}
...
\begin{proposition}\label{proposition:conditional-probability}
...
\begin{theorem}[全概率公式]\label{theorem:total-probability}
设$(\Omega,\scF,\Pr)$是一个概率空间，$A_1,A_2,\ldots\in\scF$是一列两两不相交的事件，且$\Pr(A_i)>0$，$\bigcup_{i=1}^\infty A_i=\Omega$，则对任意的$C\in\scF$，有
\[
...
注意到
\begin{align*}
    \Pr(C) = \Pr(C\cap \Omega) &= \Pr\left(C\cap\bigcup_{i=1}^\infty A_i\right)\\
    &= \Pr\left(\bigcup_{i=1}^\infty (C\cap A_i)\right)\\
    &= \sum_{i=1}^\infty \Pr(C\cap A_i).
\end{align*}
...
\begin{theorem}[贝叶斯公式]\label{theorem:bayes}
...
一个特别重要的推论被称为\textit{链式法则}，它是\textit{Bayes网络}的基础. 

\begin{corollary}[链式法则]\label{corollary:chain-rule}
设$(\Omega,\scF,\Pr)$是一个概率空间，$A_1,A_2,\ldots,A_n\in\scF$，且$\Pr(A_1\cap A_2\cap\dots\cap A_n)>0$，则
\begin{align*}
    &\Pr(A_1\cap A_2\cap\dots\cap A_n)\\
    = &\Pr(A_1)\Pr(A_2|A_1)\Pr(A_3|A_1\cap A_2)\dots\Pr(A_n|A_1\cap A_2\cap\dots\cap A_{n-1}).
\end{align*}
\end{corollary}
...
\begin{align*}
    ...
    &\dots\\
    \Pr(A_n|A_1\cap A_2\cap\dots\cap A_{n-1})&=\frac{w+nc}{w+b+nc}.
\end{align*}
    所以
\[
    \Pr(A_1\cap A_2\cap\dots\cap A_n) = \frac{w}{w+b}\cdot\frac{w+c}{w+b+c}\dots\frac{w+nc}{w+b+nc}.
\]
\end{example}
...
\begin{example}\label{example:random-variable}
继续考虑先后投两个骰子的情况，假设概率空间是$(\Omega,\scF,\Pr)$，他们的定义我们在\Cref{subsec:kolmogorov-probability} 的末尾已经讨论过了. 
...
上面例子中的$S$和$X$都是\textit{随机变量}的例子. 我们给出随机变量的定义. 

\begin{definition}[随机变量（向量），Borel函数]
设$(\Omega,\scF,\Pr)$是一个概率空间，$X:\Omega\to\R$是一个函数. 如果对任意的$A\in\scB(\R)$，都有$\{X\in A\}\in\scF$，则称$X$是一个\textbf{随机变量}. 
...
\begin{theorem}\label{theorem:random-variable}
...
\begin{proposition}\label{proposition:borel-function}
下面函数是Borel函数：
\begin{enumerate}
    \item 所有的连续函数；
...
\begin{proposition}\label{proposition:distribution-function}
...
实际上，分布函数也可以由\Cref{proposition:distribution-function} 的前三条性质给出定义，这是因为，满足前三条性质的函数恰好是某个随机变量的分布函数：

\begin{theorem}\label{theorem:distribution-function}
设$F$是$\R\to\R$的函数，满足\Cref{proposition:distribution-function} 的前三条性质. 

在概率空间$([0,1],\scB([0,1]),\lambda)$上，存在一个随机变量$X$，使得$F_X=F$.
\end{theorem}

所以，我们今后也称呼满足\Cref{proposition:distribution-function} 四条性质的函数为分布函数. 

我们看一个分布函数计算概率的简单例子. 
\begin{example}\label{example:distribution-prob}
...
\begin{align*}
    \Delta_{a_ib_i}F(x_1,x_2,\ldots,x_n)=&F(x_1,x_2,\ldots,x_{i-1},b_i,x_{i+1},\ldots,x_n)-\\
    &\qquad F(x_1,x_2,\ldots,x_{i-1},a_i,x_{i+1},\ldots,x_n).
\end{align*}
例如，对于区间$(a,b]=\{(x_1,x_2,\ldots,x_n)\in\R^n:a_i<x_i\leq b_i\}$，我们有
\[
    \Pr(X\in(a,b]) = \Delta_{a_1b_1}\Delta_{a_2b_2}\dots\Delta_{a_nb_n}F_X(x_1,x_2,\ldots,x_n).
\]

容易证明，分布函数具有如下的性质：

\begin{proposition}\label{proposition:distribution-function-n}
...
\begin{theorem}\label{theorem:distribution-function-n}
设$F$是$\R^n\to\R$的函数，满足\Cref{proposition:distribution-function-n} 的三条性质.

在概率空间$([0,1]^n,\scB([0,1]^n),\lambda)$上，存在一个随机向量$X$，使得$F_X=F$.
\end{theorem}

因此，我们今后也称呼满足\Cref{proposition:distribution-function-n} 三条性质的函数为分布函数. 

\begin{remark}
\Cref{theorem:distribution-function} 和\Cref{theorem:distribution-function-n} 其实还发挥着另一个重要的作用. 随机变量和随机向量的定义是非常抽象的，所以我们并不能很直接验证随机变量的存在性. 然而，分布函数却是极其容易构造的. 所以利用分布函数的存在性我们可以确保随机变量的存在性. 
\end{remark}

如果我们就限制在空间$([0,1]^n,\scB([0,1]^n),\lambda)$上，随机向量几乎就等同于分布函数. 在更一般的情况下，两个随机向量$X,Y$的分布函数相同时，我们称$X,Y$ \textit{同分布}，记为$X \stackrel{d}{=} Y$.

现在，我们将分布函数与概率测度联系在一起. 下面的定理说明，分布函数$F$可以自然诱导出来一个概率测度$P$，在这个测度空间上，可以定义一个随机变量$X$，使得$F_X=F$.

\begin{theorem}\label{theorem:uniqueness-of-distribution-function}
设$F:\R^n\to\R$是一个分布函数，则在可测空间$(\R^n,\scB(\R^n))$上，存在唯一的概率测度$P$，使得对任意$a_i\leq b_i$，
\[
    P\left(\prod_{i=1}^n(a_i,b_i]\right) = \Delta_{a_1b_1}\Delta_{a_2b_2}\dots\Delta_{a_nb_n}F(x_1,x_2,\ldots,x_n).
\]
此外，在概率空间$(\R^n,\scB(\R^n),Pﬁ)$上，随机向量$X(\omega)=\omega$的分布函数是$F$.
\end{theorem}
...
这张图的每一个箭头都可以反过来，从分布函数$F$出发，可以定义出一个概率测度$P$，以及一个随机变量$X$.
\begin{center}
    \includegraphics[width=0.8\textwidth]{figures/prob/prob-relations-reverse.pdf}
\end{center}

根据上面的讨论，分布函数的特性决定了随机变量的特性. 根据分布函数的不同性质，我们可以将随机变量分为不同的类型. 下面我们将讨论一些重要的类别. 

注意，有了这两个对应关系，我们不再需要讨论抽象的可测空间$(\Omega,\scF)$，我们总是把注意力放在具体的可测空间$(\R^n,\scB(\R^n))$上，此时，分布函数$F$诱导了概率测度$\Pr$，而随机变量$X(\omega)=\omega$具有分布函数$F$，因而我们可以把样本点和随机变量等同起来.

\subsection{离散型随机变量}

我们首先讨论\textit{离散型随机变量}. 离散型随机变量的分布函数$F$称之为\textit{离散型分布}，它是一个阶梯函数，它的函数值只在有限或者可数个点$x_1,x_2,\dots$上发生跳变，在$x_i$的跳变为$p_i=F(x_i)-F(x_i-)$. 这一分布函数对应的概率测度$\Pr$我们称之为\textit{离散型测度}，这种测度集中在$x_i$上，即$\Pr(X=x_i)=p_i$. 分布函数形如下图：
...
Poisson & $\Poisson(\lambda)$ &$p_k=\frac{\lambda^k}{k!}e^{-\lambda}$ & $\lambda>0$ \\
\bottomrule
\end{tabular}
}
\caption{本书中用到的离散型分布}
...
我们再来讨论\textit{连续型随机变量}，连续型随机变量的分布函数$F$称为\textit{连续型分布}，对应的概率测度$\Pr$称之为\textit{绝对连续测度}. 从名字上就可以看出，测度才是定义连续型随机变量的关键. 我们给出绝对连续测度的定义. 
...
\begin{theorem}[微积分基本定理]\label{theorem:lebesgue-differentiation-theorem}
...
\Cref{theorem:lebesgue-differentiation-theorem} 意味着，绝对连续测度对应的分布函数几乎处处可以求导，并且所得到的导函数积分回去还是原来的分布函数，也就是微积分基本定理成立. 这样的函数我们称之为\textit{绝对连续函数}.

那么，这个$f$应该如何理解呢？先不管\Cref{theorem:lebesgue-differentiation-theorem}，回到绝对连续测度，仿照导数的定义，考虑极限
...
首先，根据\Cref{theorem:lebesgue-differentiation-theorem}，$F$是连续函数，所以根据\Cref{example:distribution-prob}，$\Pr(X=x)=F(x)-F(x-)=F(x)-F(x)=0$. 所以$\Pr(X=x)$根本就是零，它和密度函数没有任何关系，所以上面这些写法都是错的. 
...
\begin{remark}
    上面的记号$\d\Pr/\d\lambda$并不是随意写出来的，我们叫它导数也不是随意的. 在测度论中，\Cref{theorem:lebesgue-differentiation-theorem} 可以被推广为\textbf{Radon-Nikodym定理}，这一定理直接保证了形如$\d\Pr/\d\lambda$的函数的存在性，这一函数被称之为\textbf{Radon-Nikodym导数}. 
\end{remark}

利用密度，我们可以很容易计算概率：
\begin{theorem}\label{theorem:continuous-prob}
...
$\chi^2$ & $\chi^2_n$ & $p(x)=\frac{1}{2^{n/2}\Gamma(n/2)}x^{n/2-1}\e^{-x/2}$，$x\geq 0$ & $n\in\N$ \\
\bottomrule
\end{tabular}
}
\caption{本书中用到的连续型分布}
...
\begin{remark}
从\Cref{theorem:lebesgue-differentiation-theorem} 来看，密度函数的定义似乎是唯一的，但是从积分的角度，如果密度函数在几个点上的值发生了变化，并不影响整个积分的值，从而也不影响求概率. 比如均匀分布$\U(a,b)$，端点$a,b$的值到底是$0$还是$1/(b-a)$并不重要，取任何一个值都是可以的.     
\end{remark}
...
我们前面已经说过，随机向量就是$\Omega\to\R^n$的映射. $n$维的随机向量可以看成$n$个随机变量的组合，可以写作$X=(X_1,\dots,X_n)^\t$. 通常，我们将$X$的分布函数称为$X_1,\dots,X_n$的\textit{联合分布}，将$X_i$的分布函数称为$X$的\textit{边缘分布}.
...
\begin{theorem}\label{theorem:continuous-vector}
设$F:\R^n\to\R$是绝对连续测度对应的分布函数，那么存在一个非负可积函数$f:\R^n\to\R$，使得对任意的$(x_1,\dots,x_n)\in\R^n$，都有
\[F(x_1,\dots,x_n)=\int_{-\infty}^{x_1}\dots\int_{-\infty}^{x_n}f(y_1,\dots,y_n)\d y_1\dots\d y_n.\]
\end{theorem}
...
同样，密度给出了概率计算的一个重要工具：
\begin{theorem}\label{theorem:continuous-prob-vec}
    设$X$是一个$n$维连续型随机向量，$f$是它的密度函数，则对任意的$B\in\scB(\R^n)$，都有
    \[\Pr(X\in B)=\int_{x\in B}f(x)\d x.\]
\end{theorem}

利用联合密度，可以计算边缘密度：
\begin{theorem}\label{theorem:continuous-marginal}
    设$X=(X_1,\dots,X_n)$是一个$n$维连续型随机向量，则对任意的$1\leq i\leq n$，都有
    \begin{align*}
        &p_{X_i}(x_i)\\
        =&\int_{(x_1,\dots,x_{i-1},x_{i+1},\dots,x_n)\in\R^{n-1}}p_X(x_1,\dots,x_n)\d x_1\dots\d x_{i-1}\d x_{i+1}\dots\d x_n.
    \end{align*}
\end{theorem}
...
由此给出了随机变量$X_1$在给定$X_2$的条件下的\textit{条件分布列}，继而给出了\textit{条件分布}. 这一定义也可以推广到$X_i$是随机向量的情况. 
...
转换思路，去尝试定义所谓的\textit{条件分布函数}：$\Pr(X_1\leq x_1|X_2=x_2)$. 考虑$\Pr(X_1\leq x_1|x_2< X_2\leq x_2+\epsilon)$，再令$\epsilon\downarrow 0$，我们有如下计算：
\begin{align*}
...
如果上面的极限存在，我们就定义它是$X_1$在给定$X_2$的条件下的\textit{条件分布}. 
...
对照\Cref{theorem:lebesgue-differentiation-theorem}，我们知道$p_{X_1,X_2}/p_{X_2}$具有密度函数的形式，所以连续性随机向量所定义的条件分布也是连续型分布，密度函数被$p_{X_1,X_2}/p_{X_2}$通常记作$p_{X_1|X_2}$，称为$X_1$在给定$X_2$的条件下的\textit{条件密度}. 
...
\[\Pr(X_{i_1}\in B_{i_1},\dots,X_{i_k}\in B_{i_k})=\Pr(X_{i_1}\in B_{i_1})\dots\Pr(X_{i_k}\in B_{i_k}),\]
...
对于离散型随机向量，它的分布函数完全由分布列决定，所以\Cref{theorem:independence} 等价于如下命题：

\begin{proposition}\label{proposition:independence-discrete}
...
对于连续型随机向量，它的分布函数完全由密度决定，所以\Cref{theorem:independence} 等价于如下命题：

\begin{proposition}\label{proposition:independence-continuous}
...
\[p(x_1,x_2)=\begin{cases}
    ...
    0,&\text{其他}.
\end{cases}\]
如果忽略了$x_i$的取值范围，我们很容易以为$p(x_1,x_2)$可以写成形式$f(x_1)f(x_2)$，所以他们相互独立. 然而事实并不是这样的！计算$X_1$的边缘密度：
...
\begin{proposition}\label{proposition:independence-conditional}
...
\begin{theorem}\label{theorem:discrete-function}
...
\begin{theorem}\label{theorem:continuous-function}
设$X$是一个连续型随机向量，$g\in C(\R^n,\R^n)$，即$\R^n\to\R^n$的连续函数，假设$\lambda(\{x\in\R^n:\det g'(x)\neq 0\})=0$，则$Y=g(X)$也是一个连续型随机向量，它的密度函数为
\[p_Y(y)=\begin{cases}
    \displaystyle\sum_{g(x)=y}|\det g'(g^{-1}(y))|^{-1}p_X(g^{-1}(y)),&\det g'(g^{-1}(y))\neq 0,\\
    0,&\text{其他}.
\end{cases}\]
其中求和号中$g^{-1}(y)$是根据相应的$x$，用反函数定理（\Cref{theorem:inverse-function}）求出局部反函数. 
\end{theorem}

这一定理的表述比较宽泛，我们可以给一个具体的例子来理解. 

\begin{example}
设$X$是一个连续型随机变量，$g(x)=x^2$，我们来计算$Y=X^2=g(X)$的密度. 直接计算\Cref{theorem:continuous-function} 中的公式，我们有
\begin{align*}
...
一般来说，\Cref{theorem:continuous-function} 中的公式并不好记，最实用的还是根据$X$和$Y$在算相同的概率这一事实直接写出 \eqref{eq:eq-prob-2}，然后根据具体的$g$来计算. 比如上面的例子，我们可以直接写出
...
\begin{proposition}\label{proposition:independence-function}
...
\begin{enumerate}
    \item 第四盘甲赢，于是甲赢得所有赌注，这样的概率是$1/2$；
    \item 第四盘乙赢，第五盘甲赢，于是甲赢得所有赌注，这样的概率是$(1/2)\times(1/2)=1/4$；
    \item 乙连赢两盘，于是乙赢得所有的赌注，这样的概率是$(1/2)\times(1/2)=1/4$. 
\end{enumerate}
...
以上的例子给了我们定义随机变量期望的基础：定义\textit{示性函数}的数学期望. 设$A$是一个事件，那么$I_A$是一个随机变量：
\[I_A(\omega)=\begin{cases}
    1,&\omega\in A,\\
    0,&\omega\notin A.
\end{cases}\]
我们称之为事件$A$的\textit{示性函数}. 示性函数的分布列是
...
第一步，定义\textit{示性函数}的数学期望\footnote{从逻辑上说，示性函数的数学期望是被定义出来的，而不是被算出来的，因为此时我们还完全没有定义什么是数学期望. }. $\E[I_A]=\Pr(A)$.

第二步，定义\textit{简单随机变量}的数学期望. 简单随机变量是形如$X=\sum_{k=1}^n x_kI_{A_k}$的随机变量，其中$x_k\in\R$，$A_k$是事件. 定义
\[\E[X]=\sum_{k=1}^n x_k\Pr(A_k).\]
这一定义与第一步是相容的：因为$I_A=1\cdot I_{A}$，所以$\E[I_A]=1\cdot\Pr(A)=\Pr(A)$. 

第三步，定义\textit{非负随机变量}的数学期望. 非负随机变量是指$X(\omega)\geq 0$对任意$\omega$成立的随机变量$X$. 考虑一列简单随机变量$\{X_n\}_{n=1}^\infty$，它满足对于每一个$\omega\in\Omega$都有当$n\to\infty$时$X_n(\omega)\uparrow X(\omega)$. 容易验证，$\E[X_n]$也是单调递增的，所以根据\Cref{proposition:monotone-bounded}，$\E[X_n]$有有限的极限或者趋于正无穷，我们都记为$\lim_{n\to\infty} \E[X_n]$.
...
第四步，定义\textit{一般随机变量}的数学期望. 考虑随机变量$X$，定义$X^+=\max\{X,0\}$，$X^-=-\min\{X,0\}$，也就是$X$的正数部分和负数部分，那么$X=X^+-X^-$. 我们有如下定义：

\begin{definition}[数学期望，Lebesgue积分，一般情形]
考虑随机变量随机变量$X$，如果$\E[X^+]$和$\E[X^-]$至少有一个有限，那么称$X$得数学期望\textbf{存在}. 此时，定义
\[\E[X]=\E[X^+]-\E[X^-]\]
为随机变量$X$的\textbf{数学期望}或\textbf{Lebesgue积分}. 
...
\begin{proposition}\label{proposition:expectation-property}
\begin{enumerate}
...
接下来，我们讨论随机变量函数的期望的求法. 假设$X$是一个$n$维随机向量，$g:\R^n\to\R$是一个Borel函数，那么$g(X)$也是一个随机变量（\Cref{theorem:random-variable}）. 计算$\E[g(X)]$有以下两种方式，我们下面分别讨论. 

第一种，利用\Cref{subsec:random-function} 中的方法，我们可以将$g(X)$的分布写出来，然后计算期望. 我们来看一个例子. 

\begin{example}\label{example:uniform-square}
...
\[X_n=\sum_{k=0}^\infty\frac{k}{n}I_{\{k/n\leq X<(k+1)/n\}}.\]
可以证明\footnote{注意，这里$g(X_n)$未必单调趋于$g(X)$了，所以这里我们其实跳了一个比较重要的步骤，即不单调趋于的时候极限也可以拿到期望外面. 由于这一步的证明比较技术，而且对本书的讨论不是特别重要，所以这里略去. 感兴趣的读者请查阅Lebesgue控制收敛定理. }
...
\begin{theorem}\label{theorem:expectation-function}
...
\begin{example}
我们重新算一次上面的\Cref{example:uniform-square}，这次我们用\Cref{theorem:expectation-function} 来计算. 设$X\sim\U(0,1)$，我们有
...
\begin{proposition}\label{proposition:expectation-indicator}
\begin{enumerate}
...
\begin{proposition}\label{proposition:independent-expectation}
设$A,B$是两个事件，那么$A$和$B$独立的充分必要条件是
\[\E[I_AI_B]=\E[I_A]\E[I_B].\]
\end{proposition}

如果我们还记得随机变量的期望是如何定义的，那么我们可以发现，\Cref{proposition:independent-expectation} 的结论可以推广到随机变量的情形：

\begin{theorem}\label{theorem:independent-expectation-2}
设$X,Y$是两个相互独立的随机变量，那么
\[\E[XY]=\E[X]\E[Y].\] 
\end{theorem}

需要注意的是，这一命题的逆命题不一定成立. 

最后，我们给一个重要的不等式. 我们说函数$g:\R\to\R$是\textit{凸函数}，如果对任意$x,y\in\R$，$t\in[0,1]$，都有
\[g(tx+(1-t)y)\leq tg(x)+(1-t)g(y).\]
关于凸函数的更多讨论，见\Cref{sec:convex-function}. 我们有如下不等式：

\begin{theorem}[Jensen不等式]\label{theorem:jensen}
...
\begin{proposition}\label{proposition:covariance-inner-product}
...
\begin{remark}
    在\Cref{proposition:covariance-inner-product} 中，我们使用了$\Pr(X=C)=1$这样的表达. 在概率论中，如果一个事件是概率$1$发生的，我们称之为\textit{几乎必然}发生. 在涉及与数学期望有关的性质的时候，我们通常只能在几乎必然的意义下成立，而不能在一般意义下成立. 比如说，“在差一个常数的意义下，协方差是一个随机变量空间的内积”这句话其实并不准确，严格来说，应该是“在差一个常数和几乎必然相等的意义下，协方差是一个随机变量空间的内积”. 也就是说，如果$\norm{X}=0$，那么$X$几乎必然为常数.
\end{remark}

协方差与独立性密切关联：

\begin{proposition}\label{proposition:independent-covariance}
设$X,Y$是两个随机变量，如果$X$和$Y$相互独立，那么
\[\cov(X,Y)=0.\]
\end{proposition}

我们称$\cov(X,Y)=0$的两个随机变量是\textit{不相关的}，用内积空间的术语，不相关的意思就是随机变量正交. 不相关的随机变量不一定是独立的，但是独立的随机变量一定是不相关的. 
...
根据Gram矩阵的性质（\Cref{proposition:gram}），$X$的协方差矩阵是一个对称半正定矩阵. 
...
\begin{theorem}\label{theorem:expectation-linear}
设$X$是一个$n$维随机向量，$A$是一个$m\times n$的矩阵，那么$\E[AX]=A\E[X]$.
\end{theorem}

接下来是协方差矩阵. 利用Gram矩阵与二次型的关系，我们容易写出如下的结论：

\begin{theorem}\label{theorem:covariance-linear}
...
\begin{theorem}\label{proposition:characteristic-function-uniqueness}
具有相同特征函数的随机变量（向量）具有相同的分布函数.
\end{theorem}

特征函数其实可以求出随机变量的分布函数：
\begin{theorem}[逆转公式]\label{proposition:characteristic-function}
...
\begin{theorem}[逆转公式，随机向量版本]\label{proposition:characteristic-function-vector}
设$X$是$n$维随机向量，它的特征函数为$f_X$，分布函数为$F_X$，那么
\begin{enumerate}
    \item 对于$F$的两个点$a<b$，满足
    \begin{align*}
        \Pr(&X_1=c_1,\dots,X_{k-1}=c_{k-1},X_k\in(a_k,b_k],\\
        &\qquad X_{k+1}=c_{k+1},\dots,X_n=c_n)=0,
    \end{align*}
    其中$c_i\in\{a_i,b_i\}$，我们有
    \begin{align*}
        &F_X(b)-F_X(a)\\
        =&\lim_{T_1,\dots,T_n\to\infty}\frac{1}{(2\pi)^n}\\
        &\qquad\int_{-T_1}^{T_1}\dots\int_{-T_n}^{T_n}\prod_{k=1}^n\frac{\exp(-it_ka_k)-\exp(-it_kb_k)}{it_k}f_X(t)\d t.
    \end{align*}
...
\begin{theorem}\label{theorem:characteristic-function-sum}
...
\begin{theorem}\label{theorem:characteristic-function-independence}
设$X_1,\dots,X_n$是$n$个随机变量，特征函数分别为$f_1,\dots,f_n$，随机向量$X=(X_1,\dots,X_n)^\t$，它的特征函数为$f$，那么$X_1,\dots,X_n$相互独立的充分必要条件是
\[f(t_1,\dots,t_n)=f_1(t_1)\dots f_n(t_n).\]
\end{theorem}

特征函数的导数可以用来计算随机变量的矩：

\begin{theorem}\label{theorem:characteristic-function-moment}
...
\[4\cdot\Pr(\{r\}|A_1)+2\cdot\Pr(\{g\}|A_1)=3.\]
在第二种情况下，期望上的分数是
\[1\cdot\Pr(\{w\}|A_2)=1.\]
\end{example}
...
\begin{definition}[基于分割的条件数学期望]\label{definition:conditional-expectation}
设$X$是一个随机变量，集族$\mathscr{A}=\{A_1,\dots,A_n\}$是$\Omega$的一个分割，满足$\Pr(A)>0$对任意$A\in\mathscr{A}$成立. $X$在$\mathscr{A}$上的\textbf{条件数学期望}是一个随机变量，记为$\E[X|\mathscr{A}]$，它的定义为
...
\begin{definition}[基于$\sigma$-代数的条件数学期望]\label{definition:conditional-expectation-2}
...
\begin{definition}[随机向量诱导的条件数学期望]\label{definition:conditional-expectation-vector}
设$X$是一个随机变量，$Y$是一个随机向量，那么$X$关于$Y$的\textbf{条件数学期望}是一个随机变量，记为$\E[X|Y]$，定义为$\E[X|\sigma(Y)]$.
\end{definition}

我们之前定义过条件分布$\Pr(X\leq x|Y=y)$，利用这一分布，我们可以求出一个条件数学期望$\E[X|Y=y]$. 下面的命题表明，这一定义和\Cref{definition:conditional-expectation-vector} 是相容的：

\begin{proposition}\label{proposition:conditional-expectation-vector}
...
\begin{proposition}\label{proposition:conditional-expectation-basic}
...
另外，重期望公式本质上就是期望版本的全概率公式（\Cref{theorem:total-probability}）. 从基于分割的条件数学期望的角度来看，这件事会更明显. 假设我们有一个分割$\mathscr{A}=\{A_1,\dots,A_n\}$，并且$\Pr(A_i)>0$，那么
...
\begin{theorem}\label{theorem:multivariate-normal-characteristic-function}
...
\begin{proposition}\label{proposition:multivariate-normal-expectation-covariance}
设$X\sim\Nor(\mu,\Sigma)$，那么
\[\E[X]=\mu,\quad\var(X)=\Sigma.\]
\end{proposition}

现在我们将这一定义推广. 注意到，$\Sigma$就是$X$的协方差矩阵，所以\Cref{theorem:multivariate-normal-characteristic-function} 中的$\Sigma$并不要求正定，只要半正定就可以定义一个特征函数了. 我们将这一定义推广到半正定矩阵的情形：

\begin{definition}[Gauss向量]\label{definition:gauss-vector}
设$\mu\in\R^n$，$\Sigma$是一个$n\times n$的对称半正定矩阵，如果随机向量$X$的特征函数为
\[f_X(t)=\exp\left(it^\t\mu-\frac{1}{2}t^\t\Sigma t\right),\]
那么称$X$是一个\textbf{Gauss向量}，记为$X\sim\Nor(\mu,\Sigma)$.
\end{definition}

如果$\Sigma$退化，$X$不能写出密度，所以也不是连续型随机向量. 但是，利用特征函数，我们依然可以研究$X$的性质. 特别是\Cref{proposition:multivariate-normal-expectation-covariance}，对于Gauss向量仍然成立. 

Gauss向量可以完全由它的期望和协方差矩阵刻画. 首先，Gauss向量的独立性等价于不相关性：

\begin{theorem}\label{theorem:gauss-vector-independence}
...
\begin{corollary}\label{corollary:gauss-vector-independence-2}
设$X_1,\dots,X_n$是$n$个Gauss向量，它们相互独立的充分必要条件是$X_1,\dots,X_n$两两不相关，即协方差矩阵$\cov(X_i,X_j)=O$，$i\neq j$.
\end{corollary}


其次，利用\Cref{theorem:expectation-linear} 和\Cref{theorem:covariance-linear}，我们可以得到如下的结论：

\begin{theorem}\label{theorem:gauss-vector-linear}
...
\begin{theorem}[Khinchin大数定律]\label{theorem:khinchin}
...
\begin{proof}
    ...
    \begin{align*}
        f_{S_n}(t)&=\E[\e^{itS_n}]\\
        &=\E[\e^{it/n(X_1+\dots+X_n)}]\\
        &=\E[\e^{it/nX_1}\dots\e^{it/nX_n}]\\
        &=\E[\e^{it/nX_1}]\dots\E[\e^{it/nX_n}]\\
        &=\left(\E[\e^{it/nX_1}]\right)^n\\
        &=\left(f_{X_1}\left(\frac{t}{n}\right)\right)^n.
    \end{align*}
    根据\Cref{theorem:characteristic-function-moment}，我们有
    \[f_{X_1}(t)=1+it\mu+o(t),\quad t\to 0.\]
    于是
    \[f_{S_n}(t)=\left(1+\frac{it\mu}{n}+o\left(\frac{t}{n}\right)\right)^n\to\e^{it\mu},\quad n\to\infty.\]
    因此，根据\Cref{proposition:characteristic-function-uniqueness}，$S_n$的分布趋于一个在$\mu$处集中的退化分布，根据定义，这意味着对任意$x$，当$n\to\infty$时，
