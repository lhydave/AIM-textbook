...
\subsection{统计决策理论} % TODO: simplify this part
...
决策$T$的一种量化指标是最小化期望意义下的损失函数：
    \[\minimize_{T}\quad\E_{X\sim P}[L(P,T(X))].\]
...
通常，我们不能知道总体$P$的分布是什么，只知道样本$X_1,\dots,X_n$，此时我们用平均损失来代替期望损失：
    \[\minimize_{T}\quad\frac{1}{n}\sum_{i=1}^n L(P,T(X_i)).\]
...
\begin{itemize}
    ...
    \item SVM损失函数（hinge损失函数）：$L(P,T(X))=\max\{0,1-Y\cdot\hat f(X)\}$. 使用此损失函数的时候，我们一般要假定$f(X)\in[-1,1]$. 关于SVM损失函数的更多内容，我们会在\Cref{sec:svm}中讨论，这里就不展开了. 
    \item 交叉熵损失函数：$L(P,T(X))=CH(\hat f(X),Y)$. 在二分类问题中，我们假设$Y$的取值为$0$或$1$，分别代表负类和正类. 函数的估计值$\hat f (X)$输出的是样本$X$属于正类的概率，而我们把$Y$看成退化分布，此时交叉熵损失函数衡量的是$\hat f(X)$和$Y$之间的差距. 关于交叉熵的更详细讨论，以及它如何在机器学习中被用来建模，请见\Cref{chap:information-theory}.
\end{itemize}
...
在深度学习中，损失函数通常还会有正则化项，比如说，如果模型的参数是$\theta$，那么我们可以使用如下的损失函数
\[\E_{X\sim P}[L(P,T(X))]+\lambda R(\theta).\]
