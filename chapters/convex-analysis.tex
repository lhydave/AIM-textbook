...
你上山采过蘑菇吗？尽管蘑菇是一种美味的食材，但是市场上吃到的大部分蘑菇都是人工养殖的. 在中国云南，一片神奇的土地上，当地人非常喜欢食用野生生长的蘑菇，俗称“野生菌”. 比起人工养殖的蘑菇，野生菌的风味要丰富和鲜美得多. 

在一众野生菌中，一种叫“松茸”的种类最为珍稀名贵，松茸因其产量少、价值高而显得极其珍贵，被誉为“万菌之王”. 迄今为止，松茸在世界上尚不能进行真正意义上的纯人工栽培，并且大多生长在高山林中，只能依靠人工寻找采摘. 

如果让你上山去采松茸，但你对松茸一无所知，你会怎么做？你只能一点一点地摸索，爬遍整座山，对着图鉴寻找和它匹配的野生菌. 这个过程非常耗时耗力，而且大概率无功而返. 实际上，松茸价格昂贵就是因为它的稀有性. 
...
首先，我们介绍决策与优化的基本原理. 我们会从损失函数出发，然后讨论优化是什么，在做什么，有什么最基本的事实. 

\subsection{损失函数}

我们考虑一个典型的情形. 在第一个情形下，给定一张图片，我们希望判断它是人、青蛙还是其他什么东西. 这是一个\textit{分类问题}，我们希望通过某种方式来判断图片的类别. 考虑一个理想的情况，我们只有$m$个类别，形成集合$\mathcal Y$，真实世界中的图片集合为$\mathcal X$，服从概率分布$P$，假设$f:\mathcal X\to\mathcal Y$表示每个样本的真实类别. 也就是说，$f(X)$是图片$X$的真实类别.

这个世界上当然有无穷多张图片，但是我们只观察到有限个图片$X_1,\dots,X_n$，它们的类别分别为$Y_1,\dots,Y_n$. 我们的任务是通过这些图片来训练一个分类器$\hat f:\mathcal X\to\mathcal Y$，使得它对整个图片集合$\mathcal X$分类的结果尽可能接近真实的分类器$f$. 换句话说，我们希望$\hat f$和$f$之间的差距尽可能小.

为了实现这一目标，我们需要一个\textit{损失函数}来衡量$\hat f$和$f$之间的差距. 这一衡量只能通过有限的样本$X_i,Y_i$来实现. 因而损失函数是一个关于样本和分类器$\hat f$的函数，记为$L(\hat{f};\{(X_i,Y_i)\}_{i=1}^n)$，在样本清楚的情况下，我们会简单记为$L(\hat{f})$. 损失函数的值越小，说明分类器$\hat f$和真实分类器$f$之间的差距越小. 

假设只有一对样本$X,Y$，我们可以写出若干种损失函数：
\begin{itemize}
    \item 平方（$L^2$）损失函数：$L(\hat{f})=(Y-\hat f(X))^2$. 使用此损失函数的时候，我们要假定$f$在实数范围取值. 
    \item $L^1$损失函数：$L(\hat{f})=|Y-\hat f(X)|$. 使用此损失函数的时候，我们要假定$f$在实数范围取值. 
    \item SVM损失函数（hinge损失函数）：$L(\hat{f})=\max\{0,1-Y\cdot\hat f(X)\}$. 使用此损失函数的时候，我们一般要假定$f(X)\in[-1,1]$. 关于SVM损失函数的更多内容，我们会在\Cref{sec:svm}中讨论，这里就不展开了. 
    \item 交叉熵损失函数：$L(\hat{f})=CH(\hat f(X),Y)$. 注意，在这个损失函数中，我们赋予了$Y$和$\hat f(X)$新的含义. 我们假设$Y$是一个类别上的概率分布，以概率$1$取值为真实类别，$\hat f(X)$是一个概率分布，表示分类器对每个类别的预测概率. 损失函数$CH(\hat f(X),Y)$表示真实类别分布和预测类别分布之间的交叉熵. 交叉熵损失函数在深度学习中非常常用，尤其是在分类问题中. 关于交叉熵的更详细讨论，以及它如何在机器学习中被用来建模，请见\Cref{chap:information-theory}.
\end{itemize}
这些损失函数会用在不同的场景之中. 

回到图片分类的问题，我们现在不是只有一对样本$X,Y$，而是有$n$对样本$\{(X_i,Y_i)\}_{i=1}^n$. 我们可以把损失函数按照加权平均的方式扩展到所有的样本上：

\[L(\hat f;\{(X_i,Y_i)\}_{i=1}^n)=\frac{1}{n}\sum_{i=1}^n L(\hat f;(X_i,Y_i)).\]

通常来说，机器学习中有两类问题：\textit{回归问题}和\textit{分类问题}. 他们两个的区别主要在于：
\begin{itemize}
...
在深度学习中，损失函数通常还会有正则化项，比如说，如果模型的参数是$\theta$，即$\hat f$可以表达为$f_\theta$，其中$\theta$是一个实数向量，那么我们可以使用如下的损失函数
\[L(f_\theta;\{(X_i,Y_i)\}_{i=1}^n)+\lambda R(\theta).\]
这里，$R(\theta)$是正则化项，$\lambda>0$是正则化系数. 正则化项的作用是防止模型过拟合，更详细的讨论请见\Cref{sec:high-dimensional-geometry}. 例如，$R(\theta)=\norm{\theta}_2^2$，这就是$L^2$正则化，这是最常用的正则化方法. 另外，$R(\theta)=\norm{\theta}_1$，这就是$L^1$正则化，它的作用是稀疏化模型参数. 通常来说，正则化系数$\lambda$越大，正则化的效果越明显，但过大的正则化系数会导致优化过分偏离原始的目标函数，从而导致模型欠拟合.
...
&\quad h_j(x)\leq 0,&\quad j=1,\dots,n,\\
&\quad x\in\Omega.
\end{alignedat}
\label{eq:opt-general}
\end{equation}
这里，s.t.（subject to）之后的内容表明了$x$取值的限制，因此被称为\textit{约束}. 其中$g_i(x)=0$和$h_j(x)\leq 0$被称为\textit{函数约束}，而$x\in\Omega$被称为\textit{集合约束}.
...
如果和本章开头的故事对比，能最小化$f$的那个$x$就是我们要找的松茸，而我们要爬的山就是 \eqref{eq:opt-general}. 不同的山自然有不同的特性，在优化这里，根据损失函数$f$、约束条件$g_i$和$h_j$的不同性质，我们可以对优化问题进行分类：
\begin{itemize}
\item 无约束优化：约束条件$g_i$和$h_j$实际上不存在，即$m=n=0$，并且$\Omega$是全空间，比如$\R^n$.
\item 有约束优化：至少存在一个约束条件，即$\max\{m,n\}\geq 1$，或者$\Omega$不是全空间. 
\item 光滑优化：损失函数和约束条件都是可微函数.\footnote{光滑这一词的含义在不同的文献中大相径庭，它可以指（连续）可微、二次（连续）可微或者无穷次可微. }
...
下面我们看几个经典的优化例子. 
\begin{example}[最小二乘法]\label{example:least-square}
...
\begin{example}[线性规划]\label{example:linear-programming}
    给定矩阵$A\in\R^{m\times n}$和向量$b\in\R^m$，考虑如下优化问题：
    \begin{alignat*}{2}
...
设$F$是有限个优化问题的集合，优化问题的变量取值范围是有限集，目标函数的可能值集也是有限集，$F$上有一个任意的概率分布.\footnote{例如考虑64位浮点数，此时，尽管优化问题的变量取值范围和目标函数的可能值集非常庞大，但都是有限集.}
...
\begin{theorem}[没有免费午餐定理，\cite{wolpertNoFreeLunch1997}]\label{theorem:no-free-lunch}
对任意优化算法$A_1,A_2$，
...
\begin{theorem}
    网格搜索算法可以找到一个$\epsilon$-近似解，其调用$\O$的次数至多为
...
\begin{theorem}\label{theorem:grid-search-lower-bound}
    设$\epsilon<L/4$，要想对任意$f$都能找到 \eqref{opt:grid-search} 的 $\epsilon$-近似解，访问$\O$的算法（零阶算法）调用$\O$的次数至少为
    \[\left\lfloor\frac{L}{2\epsilon}-1\right\rfloor^n.\]
\end{theorem}
\begin{proof}
设$p=\lfloor L/(2\epsilon)-1\rfloor$，对任意算法$A$，我们尝试构造一个函数，使得$A$调用$\O$的次数小于$p^n$时找到的$\epsilon'$-近似解满足$\epsilon'>\epsilon$，即最多是$\epsilon'$-近似解，而不是$\epsilon$-近似解.

构造思路：对任何测试点，使得$\O$总是返回$0$，于是，算法$A$只能找到$f=0$的解$\bar{x}$. 然而，这个函数的其他部分$A$都是访问不到的. 因为$A$最多可以访问$p^n-1$个点，所以我们可以在他没有访问到的点上把这个函数往下“挖”$\epsilon'$，这里$\epsilon'$略微大于$\epsilon$，这样函数的最小值就变成了$-\epsilon'$，这就是一个$\epsilon'$-近似解.
...
其中$\epsilon'$是一个略微小于$\epsilon$的正数. 这一构造如\Cref{fig:grid-func} 所示，它就是以$x^*$为中心，往下“挖”了一个锥，最底部是$(x^*,-\epsilon')$.
...
内部. 因为$1/(2p)\geq \epsilon/(L-2\epsilon)$，所以只要取$\epsilon'>\epsilon$足够接近$\epsilon$，就有$B'\subseteq B$. 所以所有测试点上$\O$都会返回$0$，这是一个$\epsilon'$-近似解. 因此$A$通过小于$p^n$次对$\O$的调用只能找到一个$\epsilon'$-近似解，$\epsilon'>\epsilon$.
\end{proof}
...
\begin{theorem}[一阶条件]\label{theorem:first-order-condition}
...
因此，
\[\inner{f'(x^*)}{y-x^*}+o(\norm{y-x^*})\geq 0.\]
注意，根据内积和长度的性质，我们可以对$y-x^*$乘任意正系数，不等式依然成立. 因此，这等价于对任意非零$s\in\R^n$，
\[\inner{f'(x^*)}{s}+o(\norm{s})\geq 0\iff \inner{f'(x^*)}{\frac{s}{\norm{s}}}+o(1)\geq 0.\]
沿着$s$的方向令$s$趋于$0$，我们得到
\[\inner{f'(x^*)}{s}\geq 0.\]
由$s$的任意性，可将$s$换为$-s$，得$\inner{f'(x^*)}{s}=0$. 由$s$的任意性，$f'(x^*)=0$.
\end{proof}
...
\begin{theorem}\label{theorem:differential-convex-equivalence}
    可微函数$f\in\calF$当且仅当$f$是凸函数.
\end{theorem}   
\begin{proof}
前面推导 \eqref{eq:def-convex} 的过程已经说明$\calF$中的函数满足 \eqref{eq:def-convex}，现在只需验证满足 \eqref{eq:def-convex} 的函数属于$\calF$，即满足三个假设.
    \begin{itemize}
...
\begin{example}\label{example:convex-function}
    \begin{itemize}
...
\begin{proposition}\label{proposition:nonnegative-combination}
    对任意$f,g\in\calF$和实数$\alpha,\beta\geq 0$，$\alpha f+\beta g\in\calF$.
\end{proposition}

另一个可以保持凸性的操作是\textit{仿射变换}. 所谓仿射变换，指的是向量空间$\R^n$到$\R^m$的映射$x\mapsto Ax+b$，其中$A$是$m\times n$矩阵，$b\in\R^m$. 仿射变换实际上就是带平移的线性映射，只是我们用变换的方式来表示它. 

\begin{proposition}\label{proposition:affine-transformation}
...
\end{equation}
Jensen不等式具有很强的几何解释：画一条$f$的割线，那么$f$的函数图像位于割线下方，如\Cref{fig:convex-Jensen} 所示. 
...
\begin{theorem}\label{theorem:convex-equivalence}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            &\qquad\inner{f'(\alpha x+(1-\alpha) y)}{x-(\alpha x+(1-\alpha) y)}.
        \end{align*}

        \item $y$取为$y$，如此得到不等式
        \begin{align*}
            f(y)&\geq f(\alpha x+(1-\alpha) y) + \\
            &\qquad\inner{f'(\alpha x+(1-\alpha) y)}{y-(\alpha x+(1-\alpha) y)}.
        \end{align*}
    \end{itemize}

    第一个不等式内积第二项为$(1-\alpha)(x-y)$，第二个不等式内积第二项为$\alpha(y-x)$. 为把梯度消去，把第一个不等式乘以$\alpha$，第二个不等式乘以$1-\alpha$，然后相加，即得 \eqref{eq:Jensen}.
...
如果函数$f$不是可微的，那么\Cref{theorem:convex-equivalence} 给了一个凸函数更加本质的定义：
\begin{definition}[凸函数]
    函数$f$满足对任意$x,y$成立 \eqref{eq:Jensen}，那么称$f$是\textbf{凸函数}.
\end{definition}

容易证明，之前陈述的可微凸函数的性质（\Cref{proposition:nonnegative-combination}、\Cref{proposition:affine-transformation}）对于这一更一般的凸函数定义也成立（见习题 \ref{exercise:convex-function-general-property}）.

扩展定义之后的凸函数包括了我们之前讲的$L^p$（$p=1,2$）损失和SVM损失，以及机器学习中用到的大部分损失函数. 在实际情况中，凸函数是一类存在快速收敛算法的函数，例如梯度下降和Newton迭代法. 因此，我们可以说，凸函数类划定了优化问题中可以快速求解的函数类. 自此，凸性成为了优化中的核心概念. 
...
上图揭示了凸集与凸函数的关系：
\begin{theorem}\label{theorem:convex-epi}
...
\begin{proposition}\label{proposition:level-set}
...
\begin{proposition}\label{proposition:convex-set-intersect}
    凸集的任意交依然是凸集.
\end{proposition}
\begin{proof}
...
从优化的角度来看，凸集本身具有\textit{最优近似性质}. 我们之前在\Cref{example:least-square} 讨论过，求点到线性空间的投影是一个优化问题. 任何一个点都可以唯一地投影到线性空间的某个点上，因此整个空间通过投影就被近似到了一个线性子空间中. 
...
\begin{theorem}\label{theorem:Chebyshev-convex}
...
凸集还有一个不平凡且重要的性质：
\begin{theorem}[分离超平面定理]\label{theorem:separation-hyperplane}
...
下面我们来证明\Cref{theorem:separation-hyperplane}.
...
\begin{remark}
    本节的内容参考了优化领域的经典著作~\cite{boydConvexOptimization2004}，这本书详细介绍了凸集和凸函数的性质，以及它们在优化问题中的应用，适合进行查阅. Chebyshev集最早是Chebyshev在多项式逼近中提出的概念~\cite{chebyshevQuestionsSmallestQuantities1947}，关于这类集合性质，请参考专著~\cite{bauschkeConvexAnalysisMonotone2017}.
\end{remark}
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    请将该分类器监督学习的过程写成优化问题.
    \item *\label{exercise:no-free-lunch}（\cite{wolpertNoFreeLunch1997}）证明\Cref{theorem:no-free-lunch}.
    ...
    \item \label{exercise:convex-function} 验证\Cref{example:convex-function} 中的函数是凸函数.
    ...
    \item \textbf{神经网络的损失函数一般没有凸性.} 我们一步一步证明这个结论：
        \begin{enumerate}
            ...
            \[NN(\mathbf{x})= \mathbf W_2\sigma(\mathbf W_1\mathbf{x}),\]
    ...
    \item \label{exercise:convex-function-general-property} 对一般定义的凸函数，验证\Cref{proposition:nonnegative-combination} 和\Cref{proposition:affine-transformation}.
    
    \item \label{exercise:convex-level-set} 对下水平集，完成以下习题. 
    \begin{enumerate}
        \item 证明\Cref{proposition:level-set}. 
    ...
    \item *\label{exercise:Chebyshev-convex} 证明\Cref{theorem:Chebyshev-convex}.
    
    \item \label{exercise:separation-hyperplane-extension} 设$C,D$是两个非空不交凸集，$C$与$D$只有一个公共点$a$，且$a$是$C$和$D$的边界点. 证明\Cref{theorem:separation-hyperplane} 对这种情况依然成立.
    \begin{hint}
