...
一种更为抽象的做法是，我们直接看算法$A$本身的性质. 此时，要想说明$A$收敛，我们要说明$A$有一个“吸收点”，即不管从何处出发，经过若干次迭代，都会收敛到这个点附近. 这样的思路是更加现代的数学方法，它被归类到了\textit{算子法}和\textit{泛函分析}中. 在这种情况下，我们把我们的研究对象$A:X\to X$称为一个\textit{算子}.

我们将看到，从算子的角度来理解收敛性，最终问题就归结到了\textit{不动点理论}. 本章将介绍两种不动点存在性定理，并介绍他们的应用.

\section{Banach不动点定理}

首先，我们需要引入一些度量空间相关的概念，更系统的讨论请参阅\Cref{chap:calculus}.
...
\begin{example}\label{example:metric-main}
...
\begin{theorem}\label{theorem:complete-metric-space-convergence}
...
\begin{example}\label{example:complete-metric-space}
\begin{itemize}
\item $L^p$度量下$\R^n$是完备的.
...
设$X$和$Y$是度量空间$(X,d_X),(Y,d_Y)$，考虑映射$f:X\to Y$和点$a\in X$，如果对于任意$\epsilon>0$，存在$\delta>0$，使得对于任意$x\in X$，有
...
考虑度量空间$(X,d)$到自身的映射$f:X\to X$（$f$被称之为算子）. 如果存在$q\in(0,1)$，使得$X$中的任何两个点$x_1,x_2$都成立不等式
...
\begin{lemma}\label{lemma:metric-continuous}
    考虑度量空间$(X,d)$，$d'$是$X\times X$上的度量，其中
    \[d'((x_1,y_1),(x_2,y_2))=d(x_1,x_2)+d(y_1,y_2).\]
    那么度量$d:X\times X\to\R$是连续映射.
\end{lemma}
\begin{proof}
    对于任意$x_1,x_2,y_1,y_2\in X$，根据三角不等式，有
    \begin{align*}
        d(x_1,y_1)-d(x_2,y_2)&=d(x_1,y_1)-d(x_1,y_2)+d(x_1,y_2)-d(x_2,y_2)\\
        &\leq d(y_1,y_2)+d(x_1,x_2).
    \end{align*}
    同理，$d(x_2,y_2)-d(x_1,y_1)\leq d(y_1,y_2)+d(x_1,x_2)$，因此
    \[|d(x_1,y_1)-d(x_2,y_2)|\leq d(y_1,y_2)+d(x_1,x_2).\]
...
\begin{theorem}[Banach不动点定理，压缩映像原理]\label{theorem:banach-fixed-point}
...
对于具体的应用，比起抽象的度量，我们会关注更具体的空间. 在$\R^n$中定义度量都有一个特点，他们都是用向量$x_1-x_2$的某种长度定义的，这种长度的概念在数学中有一个统一的抽象，即\textit{范数}. 
...
很容易看出，范数可以定义度量：$d(x_1,x_2)=\norm{x_1-x_2}$.

下面，我们举一个非$\R^n$的范数的例子.

\begin{example}[矩阵范数]\label{example:matrix-norm}
    设$X$是$n\times m$的实矩阵空间，定义$A\in X$的\textbf{2-范数}为
    \[\norm{A}_2=\sup_{x\in\R^m,\norm{x}_2=1}\norm{Ax}_2.\]
    其中$\norm{\cdot}_2$是向量的$L^2$范数. 我们来验证它是一个范数.

    \begin{enumerate}
        \item 非负性与非退化：根据定义可知，对任意矩阵$A$，$\norm{A}_2\geq 0$. 如果$A$是零矩阵，那么自然$\norm{A}_2=0$；反之，如果$\norm{A}_2=0$，那么对任意非零$x\in\R^m$，都有$\norm{Ax}_2\leq \norm{A}_2\cdot\norm{x}_2=0$，因此$Ax=0$，即$A$是零矩阵.
        \item 齐次性：对任意矩阵$A\in X$，$\lambda\in\R$和$x\in \R^m$，有
        \[\norm{\lambda A}_2=\sup_{\norm{x}_2=1}\norm{\lambda Ax}_2=|\lambda|\sup_{\norm{x}_2=1}\norm{Ax}_2=|\lambda|\norm{A}_2.\]
        \item 三角不等式：对任意矩阵$A,B\in X$，有
        \begin{align*}
            \norm{A+B}_2&=\sup_{\norm{x}_2=1}\norm{(A+B)x}_2\\
            &\leq \sup_{\norm{x}_2=1}\left(\norm{Ax}_2+\norm{Bx}_2\right)\\
            &\leq \sup_{\norm{x}_2=1}\norm{Ax}_2+\sup_{\norm{x}_2=1}\norm{Bx}_2\\
    ...
    实际上，矩阵的2-范数是一个非常重要的范数，它与矩阵的特征值有密切的关系. 具体来说，根据谱定理（\Cref{theorem:symmetric-operator-norm}），如果矩阵$A$是对称的，那么它的2-范数等于它的最大特征值的模. 具体来说，假设$A$的特征值为$\lambda_1,\dots,\lambda_n$，那么
    \[\norm{A}_2=\max_{i=1,\dots,n}|\lambda_i|.\]
\end{example}

接下来，我们在赋范空间中重新考虑压缩映射的定义. 在赋范空间$(X,\norm{\cdot})$中，考虑算子$\mathcal A:X\to X$，如果存在$q\in(0,1)$，使得对任意$x,y\in X$，
\[\norm{\mathcal A x - \mathcal A y}\leq q\cdot \norm{x-y},\]
那么$\mathcal A$是一个压缩映射.

接下来，我们从范数的角度给两个例子，说明压缩映射的应用.


\begin{example}[落在地面上的地图]
将一座公园的地图铺开在公园地面上，则地面上恰有唯一一点与地图上对应的点重合. 

设公园可以用有界的闭区域$\Omega$表示. 设地图的压缩比是$\lambda\in(0,1)$. 现在固定一个平面直角坐标系，把地图铺在区域$\Omega$内，则从$\Omega$内的点$x$（公园中的地点）到地图上对应点$x'$的变换由下面的公式给出：
\[x' = f(x) = \lambda Rx + b.\]
其中$R$和$b$分别为旋转和平移变换. 

根据旋转的定义，对任意$x\in\Omega$成立$\norm{Rx}_2=\norm{x}_2$，因此所以对任意$x,y\in\Omega$，有
\[\norm{f(x)-f(y)}_2=\norm{\lambda Rx-\lambda Ry}_2=\lambda\norm{Rx-Ry}_2= \lambda\norm{x-y}_2.\]
由于$\lambda\in(0,1)$，所以$f$是一个压缩映射.

由Banach不动点定理可知，压缩映射$f(x)$有唯一不动点$a=f(a)$.
\end{example}

\begin{example}[梯度下降的收敛性，\cite{jungFixedPointViewGradient2017}]\label{example:gradient-descent}
...
我们再假设存在常数$U\geq L$，对任意$x\in \R^n$，
    \[\lambda_{\max}(\nabla^2 f(x))\leq U,\]
...
\begin{enumerate}
    \item 首先，根据\Cref{theorem:differential-convex-equivalence}，可微凸函数$f$的最小值点充分必要地满足
    \[\nabla f(x)=0.\]
    \item 其次，显然有
...
我们只需要证明$\mathcal T^{(\alpha)}$是压缩映射，并给出压缩系数. 由有限增量原理（\Cref{theorem:lagrange-finite-multi}）:
\[\norm{\mathcal T^{(\alpha)}x - \mathcal T^{(\alpha)}y}_2 \leq \sup_{z\in(x,y)}\norm{I - \alpha \nabla^2 f(z)}_2\cdot\norm{x-y}_2.\]
这里，$(x,y)=\{(1-t)x+ty|t\in(0,1)\}$是$x$和$y$之间的线段. 

注意到$\norm{I-\alpha\nabla^2f(z)}_2$等于$I-\alpha\nabla^2f(z)$特征值的最大模. 假设$\lambda_1,\dots,\lambda_n$是$\nabla^2f(z)$的特征值，则根据\Cref{example:matrix-norm} 的讨论，
\[\norm{I-\alpha\nabla^2f(z)}_2=\max_{i=1,\dots,n}|1-\alpha\lambda_i|.\]
只要
\[0<1-\alpha\lambda_{\max} < 1\text{ 且\,} 0<1-\alpha\lambda_{\min} < 1,\]
也就是$\alpha\in(0,\lambda_{\max}^{-1})$，就会有$\norm{I-\alpha\nabla^2f(z)}_2< 1$. 根据条件，只需要$\alpha\in(0,U^{-1})$，就可以保证这一条件，与此同时，$\mathcal T^{(\alpha)}$是一个压缩映射，压缩系数为$1-\alpha L$.
\end{example}
...
\begin{theorem}\label{theorem:compact-set-iff-bounded-closed}
...
注意，\Cref{theorem:compact-set-iff-bounded-closed} 只在$\R^n$中成立，对于一般的度量空间，紧集和有界闭集不一定等价（见习题 \ref{exercise:compact-not-bounded-closed}）. 
...
\begin{theorem}[Perron-Frobenius定理]\label{theorem:perron-frobenius}
...
接下来，我们证明\Cref{theorem:perron-frobenius}，这一证明主要基于 \cite{debreuNonnegativeSquareMatrices1953} 的思路. 
...
我们断言不会有$Ax=0$. 若不然，$A$存在某一列全$0$（由$x\geq 0$和$A$非负可得）. 此时可将该$0$列交换到第一列，对应的行也交换，得到的矩阵为分块上三角，与不可约性矛盾.

可以在$S$上定义映射
\[T(x) = \frac1{\rho(x)}Ax,\]
其中$\rho(x) > 0$使得$T(x)\in S$. 具体来说，
\[\rho(x) = \sum_i (Ax)_i = \sum_{i,j} a_{ij}x_j.\]
由于$Ax$不会为$0$，因此$\rho(x)>0$.
...
\begin{proof}[\Cref{theorem:perron-frobenius} 的剩余证明]
...
\begin{enumerate}[wide, labelindent=0pt]
    \item \label{exercise:metric} 验证\Cref{example:metric-main} 中的都是度量. 
    
    \item \label{exercise:complete-metric-space} 验证\Cref{example:complete-metric-space} 中的都是完备度量空间. 
