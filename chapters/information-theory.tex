...
因此，
\begin{equation}
    \log P\sim N\log N-\sum_i N_i\log N_i- N\log m.\label{eq:logP}
\end{equation}
...
\begin{enumerate}[wide, labelindent=0pt]
    ...
    \item 本题证明 \eqref{eq:logP}.
    \begin{enumerate}
        \item 考虑两个数列$a_n$和$b_n$，他们都趋于正无穷，并且$\lim_{n\to\infty}a_n/b_n=1$，证明$\lim_{n\to\infty}\log a_n/\log b_n=1$. 这就是说，两个等价的无穷大，取对数之后依然是等价无穷大.
        \item 证明：$\sim$关系对乘法和除法是封闭的，即如果$a_n\sim b_n$，那么$a_n\cdot c_n\sim b_n\cdot c_n$以及$a_n/c_n\sim b_n/c_n$. 这里，记号$a_n\sim b_n$表示$\lim_{n\to\infty}a_n/b_n=1$.
        \item 利用前两问，证明 \eqref{eq:logP}.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \]
        然而，这个积分通常是直接写出表达式的，因为参数可能非常复杂（例如神经网络）. 为此，我们引入一个分布$q_{\phi}(z|x)$来近似后验分布，其中$\phi$是$q$的参数. 证明：
        \[
