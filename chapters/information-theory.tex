...
\end{itemize}
我们可以看到，分成两步之后，第一步试验的不确定性是$H(1/2,1/2)$，第二步试验的不确定性有一半概率是$H(1)$（上面的分支），有一半概率是$H(2/3,1/3)$（下面的分支），因而加权的不确定性是$1/2\cdot H(1)+1/2\cdot H(2/3,1/3)$. 因此第三个假设可以具体表述为
\[
...
这里，我们可以看出Shannon的哲学思想：不确定性只来自于概率分布而不是具体对象. 他的考虑具有浓厚的工程意味，正如他自己针对通信的数学理论所说：“消息是具有含义的……然而，通信的语义层面并不是工程问题所关心的. ”\cite{shannonMathematicalTheoryCommunication1948} 正是因为抽象掉了具体考虑的对象，信息论的应用才变得如此广泛. 

基于上面三个假设，Shannon证明了如下定理，这一定理直接给出了熵的概念.

\begin{theorem}[Shannon定理]\label{theorem:Shannon-thm}
    $H$满足三个假设当且仅当
...
根据对数的换底公式，可以将$C\log p_i$写为$\log_bp_i$，这里$C=1/\log b$. 于是，Shannon定理直接给出了熵的如下定义：
\begin{definition}[熵]
...
\begin{itemize}
    \item 信息论使用了概率论进行建模. 但我们已经看到，概率要么是作为频率的近似理论（频率学派），要么反映了人们对未知的信念（Bayes学派）. 无论哪种解释，都将问题简化了. 正如Kolmogorov~\cite{kolmogorovLogicalFoundationsProbability1983} 所说：“如果事情没有按照我们的预期发展，那么问题一定出在我们对于概率和真实世界的随机之间关系不清晰的认识上. ”
...
这是一个多项分布. 假设$N_i$充分大的时候，$N_i/N$呈现固定的比例$p_i$，因此，$p_i$差不多代表的是单元$i$中出现粒子的概率. 于是我们可以把$N_i$用$p_i$来表示：
    \[N_i=Np_i.\]
考虑充分大的$n$，由Stirling公式，有
\begin{align*}
    n! &= \sqrt{2\pi n}\left(\frac{n}{\e}\right)^n\left(1+O\left(\frac{1}{n}\right)\right)\\
    \implies\log n! &= \frac{1}{2}\log(2\pi n)+n\log n-n+O\left(\frac{1}{n}\right)\\
                    &= \frac{1}{2}\log n + n \log n - n + O(1).
\end{align*}
这里用了性质$\log(1+O(1/n))=O(1/n)$. 于是，
\begin{align*}
    \log P =&\log N!-\sum_i \log N_i! -N\log m\\
    =&\frac{1}{2}\log N + N\log N -N + O(1) - \\
    &\quad \sum_i\left(\frac{1}{2}\log N_i + N_i\log N_i -N_i + O(1)\right) -N\log m\\
    =&\frac{1}{2}\log N + N\log N -N - \\
    &\quad \sum_i\left(\frac{1}{2}\log(Np_i) + Np_i\log(Np_i) -Np_i\right) -N\log m + O(1)\\
    =&\frac{1-m}{2}\log N-N\sum_i p_i\log p_i -N\log m+O(1).
\end{align*}

 $\log P\sim -N\sum_i p_i\log p_i-N\log m$. 回到物理意义，真正决定系统状态的量是$p_i$，而$N$和$m$只是用来描述系统的粒子数和单元数，可以被看作是一个常数. 真正决定系统状态的概率分布的量是
 \[
    H(p_1,\dots,p_m)=-\sum_i p_i\log p_i.
\]
...
利用导数的方法，很容易证明：
\begin{proposition}\label{proposition:entropy-2prob}
...
实际上，这样的性质对于一般的分布也是成立的，我们分别将他们写在\Cref{proposition:entropy-nonnegative} 和\Cref{proposition:entropy-maximum} 中.

考虑一般分布的熵$H(p)=H(p_1,\dots,p_n)$. 我们有如下性质：
\begin{proposition}\label{proposition:entropy-nonnegative}
    $H(p)\geq 0$，等号成立当且仅当某个$p_i=1$.
\end{proposition}
\begin{proof}
这是一个典型的证明，主要的技巧是使用熵的期望形式. 考虑随机变量$X$，其分布列为$p$. 回忆Jensen不等式（\Cref{theorem:jensen}）：如果$f$是一个严格凸函数，那么
\[\E[f(X)]\geq f(\E[X]).\]
等号成立当且仅当$X$是常数.
...
\begin{proposition}\label{proposition:entropy-adjust}
...
\begin{proof}
    ...
    \begin{align*}
       ...
       \leq &H(p_1+p_2,p_3,\dots)+(p_1+p_2)H\left(\frac{p_1'}{p_1'+p_2'},\frac{p_2'}{p_1'+p_2'}\right)\quad(\text{\Cref{proposition:entropy-2prob}})\\
       =&H(p_1',p_2',p_3,\dots).\quad(\text{假设三})
    \end{align*}
\end{proof}

\begin{proposition}\label{proposition:entropy-maximum}
    当且仅当$p_1=\dots=p_n=1/n$时$H$取得最大值$\log n$.
\end{proposition}
\begin{proof}
    用反证法，假设达到最大值的时候存在$p_k\ne 1/n$，那么，因为$\sum_i p_i/n=1/n$，根据鸽巢原理，则必有$i,j$满足
    \[p_i<1/n<p_j.\]
    根据\Cref{proposition:entropy-adjust}，我们可以将$p_i$和$p_j$替换为$1/n$和$p_i+p_j-1/n$，而$H$增大，这与假设矛盾. 因此，$p_1=\dots=p_n=1/n$是$H$取得最大值的必要条件. 

    另一方面，因为$H$连续，所以根据Weierstrass最值定理（\Cref{theorem:weierstrass}），$H$一定有最大值，所以$p_1=\dots=p_n=1/n$也是$H$取得最大值的充分条件.
\end{proof}

至此，\Cref{proposition:entropy-nonnegative} 和\Cref{proposition:entropy-maximum} 证明了一般情形的\Cref{proposition:entropy-2prob}. 在等可能的时候不确定性最大，熵最大；在确定事件的时候不确定性最小，熵最小. 所以熵是符合直观的定义. 
...
对于联合分布，我们自然可以讨论它\textbf{边缘分布的熵}
\[H(X)=-\E[\log p_X(X)]=-\sum_x p_X(x)\log p_X(x).\]
\[H(Y)=-\E[\log p_Y(Y)]=-\sum_y p_Y(y)\log p_Y(y).\]
这里，边缘分布通过对联合分布求和得到：
    \[
...
\begin{proposition}\label{proposition:entropy-joint-marginal}
    对任意随机变量$X,Y$，$H(X)\leq H(X,Y)\leq H(X)+H(Y)$. 第一个等号成立当且仅当以概率$1$，$Y$被$X$完全决定，也就是$Y$是$X$的函数；第二个等号成立当且仅当$X$和$Y$是独立的.
\end{proposition}

第一个不等号的证明等价于证明后面定义的条件熵的\Cref{proposition:conditional-entropy-nonnegative}，而第二个不等式等价于信息不等式（\Cref{theorem:information-inequality}），这里就都不给出证明了.

这两个不等式实际上非常符合我们对信息的直观. $H(X,Y)$是$X$和$Y$的联合不确定性，$H(X)$是$X$的不确定性，$H(Y)$是$Y$的不确定性. 
\begin{itemize}
    \item 因此，两个不同来源$X$和$Y$的不确定性一定不小于单个来源$X$的不确定性，除非$Y$完全被$X$决定. 
    \item 另一方面，两个不同来源$X$个$Y$的不确定性可能会有一些共有的部分，所以联合的不确定性一定不大于两个单独的不确定性之和，除非两个来源是独立的.
\end{itemize}

有了两个随机变量，我们就可以讨论“条件”的概念. 具体来说，我们可以把试验分为两步，第一步观测$X$，第二步观测$Y$，那么，第二步所产生的熵就是已经知道第一步结果之后的熵，即：
\begin{align*}
    H(Y| X=x)&=-\E_Y[\log p_{Y| X}(Y|x)]\\
    &=-\sum_{j}p_{Y|X}(j|x)\log p_{Y|X}(j|x),
\end{align*}
其中$p_{Y|X}(j|x)=p(x,j)/p_X(x)$. 当我们知道了$X=x$之后，对$Y$的观测就消除了部分的不确定性，因此根据我们对于不确定性和信息关系的讨论，从$X=x$中获得的关于$Y$的信息是
\[I(X=x;Y)=H(Y)-H(Y|X=x).\]
考虑一个特殊情况，$Y=X$，那么刚刚的讨论就变成了自己从自己身上获得的信息，或者说知道$X=x$带来的信息量. 首先有
\[p_{X|X}(i|x)=\begin{cases}
    1,&i=x\\
    0,&i\ne x.
\end{cases}\]
因此，
\begin{align*}
    H(X| X=x)&=-\sum_{j}p_{X|X}(j|x)\log p_{X|X}(j|x)\\
    &=-1\log 1=0.
\end{align*}
于是，
...
\begin{align*}
    ...
    =&-\sum_x p_{X}(x)\sum_{j}p_{Y|X}(j|x)\log p_{Y|X}(j|x)\\
    =&-\sum_{x,j}p(x,j)\log p_{Y|X}(j|x)\\
    =&-\E_{X,Y}[\log p_{Y| X}(Y|X)].
\end{align*}
换言之，我们现在进一步假定$X$也是不知道的，于是$H(Y|X)$就是平均上来说第二步中$Y$的不确定性. 

条件熵和熵有着类似的性质：
\begin{proposition}\label{proposition:conditional-entropy-nonnegative}
    $H(Y|X)\geq 0$，等号成立当且仅当以概率$1$，$Y$被$X$完全决定，也就是$Y$是$X$的函数.
\end{proposition}
\begin{proof}
    仿照\Cref{proposition:entropy-nonnegative} 的证明. 因为$-\log(\cdot)$是严格凸函数，所以根据Jensen不等式
    \begin{align*}
        H(Y|X)=&\E_{X,Y}[-\log p_{Y| X}(Y|X)]\\
        \geq&-\log\E_{Y|X}[p_{Y|X}(Y|X)]\\
        \geq& -\log 1=0.
    \end{align*}
    等号成立当且仅当$p(Y|X)$以概率$1$取$1$，即$Y$被$X$完全决定，也就是$Y$是$X$的函数.
\end{proof}
类似地，我们可以考虑平均上$X$中包含的关于$Y$的信息量：
\[\E[I(X=x;Y)]=H(Y)-\E_{x\sim X}[H(Y|X=x)]=H(Y)-H(Y|X).\]
与之相对应地，平均上$Y$中包含的关于$X$的信息量为
\[\E[I(Y=y;X)] = H(X) - \E_{y\sim Y}[H(X|Y=y)] = H(X) - H(X|Y).\]
一个自然的问题是，二者相互包含的信息量是什么关系？根据概率的链式法则，$p(x,y)=p_{X|Y}(x|y)p_Y(y)$，带入$H(X,Y)$的定义得熵的链式法则：

\begin{proposition}\label{proposition:entropy-chain-rule}
...
\begin{proposition}\label{proposition:information-doesnt-hurt}
    $H(X)-H(X|Y)\geq 0$，等号成立当且仅当$X$和$Y$相互独立.
\end{proposition}

我们将在\Cref{sec:kl-divergence}看到，\Cref{proposition:information-doesnt-hurt} 就是K-L散度信息不等式的一个特例，所以我们就不在这里给出证明了. \Cref{proposition:information-doesnt-hurt} 表明知道任何信息都不会增加不确定性，这个原理被称为“Information doesn't hurt.”
...
\[H(X,Y|Z)=H(X,Y,Z)-H(Z).\footnote{严格上说，应该写成$H((X,Y)|Z)$. 为简便起见，我们规定表示条件的竖线“$|$”具有最低的优先级，这样就可以省略括号了. }\]
类似地，我们可以定义互信息为
\[I(X,Y;Z)=H(X,Y)-H(X,Y|Z).\footnote{同理，严格的写法应该是$I((X,Y);Z)$，我们规定表示互信息的分号“$;$”具有高于竖线“$|$”但低于其他一切的优先级，这样我们就把括号去掉了. }\]
他们的含义以及性质和二元情形类似.

同样，我们可以定义\textbf{条件互信息}为
\[I(X;Y|Z)=H(X|Z)-H(X|Y,Z),\]
表明$Z$已知时候$Y$给$X$带来的平均信息增益. 类似互信息，条件互信息有如下性质：
\begin{proposition}\label{proposition:conditional-mutual-information}
    条件互信息满足以下性质：
    \begin{enumerate}
    ...
    \end{enumerate}
\end{proposition}
前两条性质说明，条件在$Z$上，$X$和$Y$的互信息同样是非负的、对称的. 第三条性质是链式法则对联合分布的推广. 最后一条性质说的其实是，在平均的意义下，给定$Y$的时候，知道$X$所能够得到的额外信息量就是$H(X|Y)$. 这一命题的证明和前面都非常相似，见习题 \ref{exercise:conditional-mutual-information}. 
...
\end{align*}
根据\Cref{proposition:information-doesnt-hurt}，$I(X;Y)\geq 0$，等号成立当且仅当$X,Y$相互独立，即$p(x,y)=p_X(x)p_Y(y)$. $X,Y$之间的互信息越大，说明他们之间的关联越强，分布越不独立，$p(x,y)$越不接近$p_X(x)p_Y(y)$. 
...
\begin{proposition}\label{proposition:mutual-information-kl-divergence}
...
\begin{theorem}[信息不等式]\label{theorem:information-inequality}
...
信息不等式表明，K-L散度虽然不是度量，但却是非负的，因而确实可以被作为熵，用来衡量“额外的不确定性”. 此外，\Cref{proposition:information-doesnt-hurt} 是信息不等式的直接推论. 利用类似的方法，我们可以证明条件互信息的非负性（即\Cref{proposition:conditional-mutual-information} 中的第一条）.

接下来我们叙述并证明数据处理不等式.
\begin{theorem}[数据处理不等式]\label{theorem:data-processing-inequality}
...
因此，我们需要引入一个额外的$W$，然后让$Z$同时依赖$Y$和$W$，这样$Z$有额外的信息注入，才能增加信息量. 例如$Z$是对$Y$进行的人工（引入$W$）标注，那么$I(X;Z)$就会比$I(X;Y)$更大，这样的数据处理才是有意义的，典型的例子如人类反馈强化学习（RLHF~\cite{ouyangTrainingLanguageModels2022}），这是现在改善大模型表现的一种有效方法.

另一种带来额外信息的方式是，我们引入了一系列规则（引入$W$）来计算$Z$的奖励，然后利用强化学习的方法来训练$Y$，这样的数据处理也是有意义的. 例如，DeepSeek-R1~\cite{deepseek-aiDeepSeekR1IncentivizingReasoning2025} 模型训练的过程中，对于编程题和数学题有关的训练数据，都知道正确答案，因此可以直接通过大模型有没有输出正确答案来计算奖励，进行强化学习. 通过这样的办法，DeepSeek-R1可以超越没有做强化学习的基础模型. 
...
\begin{itemize}
    ...
    \item 为了给出上界，我们需要给一种特定的编码方式，然后$h(\omega)$自然不会超过它. 例如，我们可以用二进制按顺序编码每一个$k_i$，然后再给定$\Omega$的一个特定顺序（例如字典序）编码每一条$\omega$. 因为$k_i\leq k$，每个$k_i$最多需要$\log_2 k$比特来表示，所以按顺序表示所有的$k_i$至多需要$s\log_2 k$比特；给定特定$k_i$之后，每个$\omega$至多需要$\log_2|\Omega|$比特来表示是第几个$\omega$. 
\end{itemize}
于是，我们需要的比特数为
\[\log_2\frac{k!}{k_1!\cdot\dots\cdot k_s!}\leq h(\omega)\leq s\log_2k+\log_2\frac{k!}{k_1!\cdot\dots\cdot k_s!}.\]
这刚好和我们在统计力学中推导熵的过程是一致的！

假设消息足够的长，$x_i$出现的频率逐渐接近$p_i$，那么，类似统计力学推理熵的过程，我们可以知道，
...
然而，长为$k$的所有消息数目为
\[s^k=\e^{k\log s}.\]
根据\Cref{proposition:entropy-maximum}，只有当所有$p_i$相等的时候$N(k)$才会达到这一量级. 从这个意义上说，\textit{熵所刻画的信息量定量刻画了数据压缩可能的极限.}

以上关于信息编码下界以及数据压缩的讨论，在更一般的情况下也成立，此时这样的性质被称为\textit{渐近等分性}. 而这一性质成立对应的结果被称为\textit{Shannon-McMillan-Breiman定理}，它的陈述以及证明都需要用到更多随机过程的知识，这里就不再给出了，感兴趣的读者请参阅Shannon的工作~\cite{shannonMathematicalTheoryCommunication1948}、McMillan的工作 \cite{mcmillanBasicTheoremsInformation1953} 和Breiman的工作 \cite{breimanIndividualErgodicTheorem1957}. 

\subsection{K-L散度、交叉熵与编码}

我们在K-L散度的定义中提到了它的另一个名字——相对熵. 实际上，这可以从编码中看出来. 假设事实上消息中字母的分布是$p_1$，那么期望上编码单位长度消息需要的比特数是
\[H(p_1)=-\E_{X\sim p_1}[\log p_1(X)].\] 
如果我们错误地认为消息中字母的分布是$p_2$并使用最优编码，那么实际上期望编码单位长度消息需要的比特数是
\[-\E_{X\sim p_1}[\log p_2(X)].\]
由于错误的认识所产生的额外编码长度是
\[\E_{X\sim p_1}[-\log p_2(X)-(-\log p_1(X))]=D_\KL(p_1\Vert p_2).\]
...
我们在这一部分给出Shannon定理（\Cref{theorem:Shannon-thm}）的证明. 整体上的思路是：
\begin{enumerate}
...
对第二步，我们需要求出$f(n)=H(1/n,\dots,1/n)$的表达式. 我们要利用第一步的结果，首先，根据假设二，$f(n)$是单调递增的函数. 然后，考虑$mn$个等可能试验，我们可以将它分成两步试验，第一步有$m$种等可能的结果，而在每一种结果之下，第二步有$n$种等可能结果. 根据假设三，
\[
...
\begin{enumerate}[wide, labelindent=0pt]
    \item \label{exercise:conditional-mutual-information} 证明\Cref{proposition:conditional-mutual-information}. 
    ...
    \begin{enumerate}
        ...
        \item 证明：对给定的$a>0$，$\lim_{x\to 0+}a\log (a/x)=+\infty$，因此我们规定了$a\log(a/0)=+\infty$.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \[\frac{1}{N}S(\theta ; x_1, \ldots, x_N).\]
        \item 证明：以概率$1$成立
        \[\lim_{N\to\infty}\frac{1}{N}S(\theta ; x_1, \ldots, x_N)=CH(B(1,p)\| B(1,\theta)).\]
        因此，当样本数量足够大的时候，$S/N$就是真实分布与模型分布的交叉熵.
        \item 证明: 对任意$N$，
        \[\arg\max_\theta L(\theta ; x_1, \ldots, x_N)=\arg\min _\theta S(\theta ; x_1, \ldots, x_N),\]
        也就是说，\textit{最大似然估计等价于最小化交叉熵}.
    \end{enumerate}

    \item \label{exercise:diffusion-model-loss}
    在生成模型中，我们假设数据$x$是由隐变量$z$生成的. 下面我们来推导VAE（Variational Auto-encoder）与扩散模型的损失函数.
    \begin{enumerate}
        \item （VAE，\cite{kingmaAutoEncodingVariationalBayes2022,kingmaSemisupervisedLearningDeep2014}）设考虑数据$x$，它是通过如下两步生成的：首先从先验分布$p_{\theta^*}(z)$中采样$z$，然后从条件分布$p_{\theta^*}(x|z)$中采样$x$. 我们可以把$z$理解成$x$的某种编码或者压缩的表示方式. 
        
        假设$p_{\theta^*}(z)$和$p_{\theta^*}(z|x)$分别来自分布族$\{p_{\theta}(z)\}_\theta$和$\{p_\theta(x|z)\}_\theta$，其中$\theta$是参数. 比如，我们可以把$\theta$想像成神经网络的参数，而神经网络在拟合上述真实参数. 真实参数$\theta^*$和中间隐变量$z$是未知的. 
        
        VAE有若干可能的任务，例如，通过最大化似然$p_{\theta}(x)$来近似$\theta^*$，或者近似估计真实的后验分布$p(z|x)$用来做编码，或者直接近似分布$p(x)$用来做数据生成. 对于最大似然的任务，我们可以将似然$p_{\theta}(x)$写成：
        \[
            p_{\theta}(x) = \int p_\theta(z)p_\theta(x|z)dz.
        \]
        然而，这个积分通常是很难直接写出表达式的，因为参数可能非常复杂（例如神经网络）. 为此，我们引入一个分布$q_{\phi}(z|x)$来近似后验分布，其中$\phi$是$q$的参数. 证明：
        \[
            \log p_{\theta}(x) \geq \E_{z\sim q_{\phi}(z|x)}[\log p_{\theta}(x|z)] - D_\KL(q_{\phi}(z|x)\Vert p_{\theta}(z)).
        \]
        于是，最大化似然可以通过优化上面的ELBO（Evidence Lower Bound）来实现，此时我们需要同时优化$\theta$和$\phi$.
        ...
        借助上一问的结论，证明损失函数可以写成：
        \begin{align*}
            \mathcal{L} =&\underbrace{\E_{x_0\sim q(x_0)}[D_{\KL}\left(q\left(x_T | x_0\right) \| p\left(x_T\right)\right)]}_{L_T}+\\
            &\qquad\sum_{t>1} \underbrace{\E_{x_0,x_t\sim q(x_0,x_t)}[D_{\KL}\left(q\left(x_{t-1} | x_t, x_0\right) \| p_\theta\left(x_{t-1} | x_t\right)\right)]}_{L_{t-1}}+\\
            &\qquad\underbrace{\E_{x_0,x_1\sim q(x_0,x_1)}[-\log p_\theta\left(x_0 | x_1\right)]}_{L_0},        
        \end{align*}
