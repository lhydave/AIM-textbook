...
\begin{proposition}\label{prop:entropy-joint-marginal}
    对任意随机变量$X,Y$，$H(X)\leq H(X,Y)\leq H(X)+H(Y)$. 第一个等号成立当且仅当以概率$1$，$Y$被$X$完全决定，也就是$Y$是$X$的函数；第二个等号成立当且仅当$X$和$Y$是独立的.
\end{proposition}

第一个不等号的证明等价于证明后面定义的条件熵的\Cref{prop:conditional-entropy-nonnegative}，而第二个不等式等价于信息不等式（\Cref{thm:information-inequality}），这里就都不给出证明了.
...
\end{proposition}
\begin{proof}
    仿照\Cref{prop:entropy-nonnegative} 的证明. 因为$-\log(\cdot)$是严格凸函数，所以根据Jensen不等式
    \begin{align*}
