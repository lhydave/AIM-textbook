...
\begin{proposition}\label{prop:entropy-joint-marginal}
    对任意随机变量$X,Y$，$H(X,Y)\geq H(X)$，等号成立当且仅当$X$和$Y$独立.
\end{proposition}

这一性质的证明等价于证明后面定义的条件熵的\Cref{prop:conditional-entropy-nonnegative}，这里就不给出了.

这一关系的解读如下：$H(X,Y)$是$X$和$Y$的联合不确定性，$H(X)$是$X$的不确定性. 因此，两个不同来源的不确定性一定不小于单个来源的不确定性. 等号成立当且仅当$X$和$Y$是独立的，也就是说，$X$的取值不会对$Y$的取值产生影响，反之亦然.
