...
因此，我们需要引入一个额外的$W$，然后让$Z$同时依赖$X$和$W$，这样$Z$有额外的信息注入，才能增加信息量. 例如$Z$是对$Y$进行的人工（引入$W$）标注，那么$I(X;Z)$就会比$I(X;Y)$更大，这样的数据处理才是有意义的，典型的例子如人类反馈强化学习（RLHF~\cite{ouyangTrainingLanguageModels2022}），这是现在改善大模型表现的一种有效方法.
