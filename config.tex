...
% Fix cleveref+ctex on recent LaTeX kernels:
% ctex defines \p@chapter/\p@section/... as 1-arg macros, but cleveref v0.21.4
% builds \cref@currentlabel assuming \p@<counter> takes 0 args, which leaves a
% stray \endcsname and breaks \Cref/\cref ("Extra \endcsname").
\makeatletter
...
\def\refstepcounter@optarg[#1]#2{%
  ...
  \protected@edef\cref@currentlabel{%
    ...
    \csname p@#2\expandafter\endcsname\csname the#2\endcsname}}%

% LaTeX's "firstaid/cleveref" hook (newer kernels) also updates
% \cref@currentlabel at each \label; patch it to use the kernel-safe
% \p@<counter> handling as well.
\@ifundefined{firstaid@cref@updatelabeldata}{}{%
  \renewcommand\firstaid@cref@updatelabeldata[1]{%
    \cref@constructprefix{#1}{\cref@result}%
    \@ifundefined{cref@#1@alias}%
      {\def\@tempa{#1}}%
      {\def\@tempa{\csname cref@#1@alias\endcsname}}%
