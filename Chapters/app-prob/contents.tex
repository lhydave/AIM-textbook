...
设$F$是$\R\to\R$的函数，满足\Cref{prop:distribution-function} 的四条性质. 
...
对于$\R^n\to\R$型的分布函数$F$，我们也有类似的讨论. 此时有多个维度，所以我们需要引入一个差分算子$\Delta_{a_ib_i}$，它的作用是
\begin{align*}
...
总结来说，随机元$X$、概率测度$P_X$和分布函数$F_X$的关系如图：
\begin{center}
\begin{tikzpicture}
...
\[p_{Z,Y}(z,y)=p_{X,Y}(x,y)\left|\frac{\partial(z,y)}{\partial(x,y)}\right|^{-1}=p_{X,Y}(z-y,y).\]
于是，$Z$的边缘密度为
...
设$X_1,\dots,X_n$是$n$个随机向量，第$i$个的维数是$n_i$，$g_i$是$\R^{n_i}\to\R^{m_i}$的连续函数（更一般地，Borel函数），$Y_i=g_i(X_i)$，如果$X_1,\dots,X_n$相互独立，那么$Y_1,\dots,Y_n$也相互独立. 
\end{proposition}

\section{随机变量的数字特征，条件数学期望}\label{sec:expectation}

分布函数或者随机变量依然是一个映射，研究起来还是会比较复杂. 我们希望能够用一些数字来刻画随机变量的特征，这样可以进一步简化问题. 在这一节中，我们将介绍随机变量的数字特征，以及条件期望. 
...
示性函数建立了概率和数学期望的联系. 下面，我们来定义一般随机变量的数学期望，这一定义的过程反映了一种数学的思想：用简单东西的极限去逼近复杂的东西. 

第一步，定义\emph{示性函数}的数学期望\footnote{从逻辑上说，示性函数的期望是被定义出来的，而不是被算出来的，因为我们还完全没有数学期望的定义. }. $\E[I_A]=\Pr(A)$.
...
\end{definition}
可以证明，这一定义不依赖于$\{X_n\}_{n=1}^\infty$的选取，因而是良定义的. 
...
设$X$是一个连续型随机变量，密度为$p$，分布为$F$. 我们来计算$\E[X]$，与其说是计算，不如说重新推导一遍第三步的过程. 首先，我们将$X$取值范围离散化，每$1/n$一段，$X$的值都压到形如$k/n$的点上，这样就转化为了一个离散型随机变量：
...
我们已经给出了数学期望的定义，下面我们罗列一些数学期望的性质，我们都不给出证明. 
...
接下来，我们讨论随机变量函数的期望的求法. 假设$X$是一个$n$维随机向量，$g:\R^n\to\R$是一个连续函数（更一般地，Borel函数），那么$g(X)$也是一个随机变量. 计算$\E[g(X)]$有以下两种方式，我们下面分别讨论. 
...
设$X$是一个$n$维随机向量，有有限的期望，$g:\R^n\to\R$是一个Borel函数，那么$\E[g(X)]$存在，且
...
\begin{example}
我们重新算一次上面的\Cref{ex:uniform-square}，这次我们用\Cref{prop:expectation-function} 来计算. 设$X\sim\U(0,1)$，我们有
...
容易验证，协方差是一个对称正定的双线性型，所以它确实形成了随机变量的内积. 按照内积空间的性质，随机变量的范数自然就是它的方差. 
...
\begin{enumerate}
    ...
    \begin{align*}
        &F_X(b)-F_X(a)\\
        =&\lim_{T_1,\dots,T_n\to\infty}\frac{1}{(2\pi)^n}\int_{-T_1}^{T_1}\cdots\int_{-T_n}^{T_n}\frac{\exp(-it^\t a)-\exp(-it^\t b)}{it^\t}f_X(t)\d t.
    \end{align*}
...
这一计算对示性函数解释了“按照$A_i$上的条件概率算期望”. 按照随机变量数学期望的定义，在一般情况下，我们也可以按照这一思路来理解. 
...
设$X$是一个非负随机变量，$\F$是$\Omega$的一个$\sigma$-代数，随机变量$\E[X|\F]$被称为$X$关于$\F$的\textbf{条件数学期望}，如果它满足
\begin{enumerate}
    \item 对任意$B\in\B(\R)$，$\{\E[X|\F]\in B\}\in\F$；
    \item 对任意$A\in\F$，$\E[XI_A]=\E[\E[X|\F]I_A]$.
\end{enumerate}

设$X$是一个一般的随机变量，如果
\[\min\{\E[X^+|\F],\E[X^-|\F]\}<+\infty,\]
那么$X$关于$\F$的\textbf{条件数学期望}是一个随机变量，记为$\E[X|\F]$，定义为
\[\E[X|\F]=\E[X^+|\F]-\E[X^-|\F].\]
\end{definition}

这一定义分成两部分，这类似于我们在定义数学期望时候做的事情：先定义非负的情况，再定义一般情况. 对于非负随机变量的定义，第一条要求是说，“$\E[X|\F]$取合理的值”这件事情是可以用$\F$中事件描述的，这和随机变量的定义是类似的；而第二条则反映了“条件”的性质，也就是我们刚刚讨论的 \eqref{eq:conditional-expectation} 式. 
...
\end{definition}
我们说过，$\{X\in B\}$表示“$X$取合理的值”. 在之前定义随机变量的时候，我们要求取合理的值是一个事件，这里则是更加简单粗暴，我们直接定义$\{X\in B\}$是一个事件. 接下来，我们可以定义随机向量诱导的条件数学期望：
...
我们不满足于$\E[X|Y=y]$，而是费尽周章定义条件期望$\E[X|Y]$，是因为他通常来说更好用，特别是在随机过程中，他能给我们很多公式直观上的含义. 这一点在\Cref{chap:markov-chain} 中会有很多体现. 
...
\begin{proposition}\label{prop:gauss-vector-independence}
设$X=(X_1,\dots,X_n)\sim\Nor(\mu,\Sigma)$，那么$X_1,\dots,X_n$相互独立的充分必要条件是$X_1,\dots,X_n$两两不相关，即$\Sigma$是一个对角矩阵. 
\end{proposition}

需要注意的是，如果$X$是正态分布，$Y$是正态分布，这并不意味着$(X,Y)$是Gauss向量，因为$X,Y$之间的联合分布可能会长得各式各样，不一定能够形成一个Gauss向量，因而并不能用不相关来作为独立性的判据. 因此，在一般情况下，我们必须要验证$(X_1,\dots,X_n)$是Gauss向量，然后才能断言不相关等价于独立. 
...
其次，利用\Cref{prop:expectation-linear} 和\Cref{prop:covariance-linear}，我们可以得到如下的结论：
