...
\begin{example}\label{ex:metric}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Minkowski度量（$L^p$度量）：$d(x_1,x_2)=(\sum_{i=1}^n|x_1^i-x_2^i|^p)^{1/p}\ (p\geq 1)$. \index{度量!Minkowski～}\index{度量!$L^p$～}
...
对于实值函数来说，我们还需要比较在极限情况下两个函数的\emph{渐近大小}，这就是$o$和$\O$符号. 我们先给出这一概念在序列上的定义：
